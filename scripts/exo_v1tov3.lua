...
local function load_v1(path)
	...
	if not f then
		die("loadfile失败: %s (%s)", path, err or "?")
	end

	local ok, exo = pcall(f)

	if not ok then
		die("执行失败: %s (%s)", path, exo or "?")
	end
...
local function is_v1(exo)
	return exo and exo.animations and exo.animations[1] and exo.animations[1].frames and exo.animations[1].frames[1] and exo.animations[1].frames[1].parts ~= nil
end
	...
	for _, a in ipairs(exo.animations or {}) do
		...
		for _, af in ipairs(a.frames or {}) do
			...
			for _, ap in ipairs(af.parts or {}) do
				...
				tf[#tf + 1] = {1, idx, ap.alpha or 1, xf.x or 0, xf.y or 0, xf.sx or 1, xf.sy or 1, xf.r or 0, xf.kx or 0, xf.ky or 0}
			end
			...
			for _, aa in ipairs(af.attachPoints or {}) do
				...
				tf[#tf + 1] = {8, idx, aa.alpha or 1, xf.x or 0, xf.y or 0, xf.sx or 1, xf.sy or 1, xf.r or 0, xf.kx or 0, xf.ky or 0}
			end
