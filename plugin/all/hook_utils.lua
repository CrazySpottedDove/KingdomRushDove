...
function hook_utils.HOOK(obj, fn_name, handler, priority)
	if not obj then
		log.error(_("PLUGIN_UI_HOOK_NIL_OBJECT"), fn_name, debug.traceback())
		return
	end

	if not obj[fn_name] then
		log.error(_("PLUGIN_UI_HOOK_MISSING_FUNCTION"), fn_name, debug.traceback())
		return
	end

	if type(handler) ~= "function" then
		log.error(_("PLUGIN_UI_HOOK_HANDLER_NOT_FUNCTION"), fn_name, debug.traceback())
		return
	end
