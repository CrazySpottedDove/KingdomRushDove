...
function scripts.enemy_mixed_water.update(this, store)
	...
	simulation:queue_insert_entity(water_trail)
			...
			if enemy then
				...
				if enemy then
					...
					if b.flight_time_min and b.flight_time_factor then
						...
						b.flight_time = b.flight_time_min + dist / at.range * b.flight_time_factor
					end
...
function scripts.arrow_missile.update(this, store)
	...
	if (not target or target.health.dead) or retarget_for_hit_miss or retarget_for_next then
		...
		elseif retarget_for_next then
			...
			local _, targets = U.find_foremost_enemy_in_range_filter_on(this_pos, retarget_range, nil, b.vis_flags, b.vis_bans, function(e)
			...
			if targets then
				...
				for i = 1, max_id do
					...
					local d_angle = V.angleTo(b.speed.x, b.speed.y, new_target.pos.x - this_pos.x, new_target.pos.y - this_pos.y)

					if math.abs(d_angle) < max_seek_angle or i == max_id then
						target = new_target
		...
		if target then
			...
			while V.dist2(this_pos.x, this_pos.y, b.to.x, b.to.y) > mspeed * mspeed * store.tick_length * store.tick_length do
				...
				this_pos.x, this_pos.y = this_pos.x + b.speed.x * store.tick_length, this_pos.y + b.speed.y * store.tick_length
...
function scripts.enemy_bomb.update(this, store)
	...
	if target and target.vis and U.flag_has(target.vis.flags, F_FLYING) then
		...
		targets = table.filter(store.soldiers, function(_, e)
			return e and not e.health.dead and band(e.vis.flags, b.damage_bans) == 0 and band(e.vis.bans, b.damage_flags) == 0 and U.is_inside_ellipse(e.pos, b.to, b.damage_radius)
		end)
	end
...
function scripts.tunnel.update(this, store)
	...
	while true do
		for _, e in pairs(store.enemies) do
			if e and not e.health.dead and e.main_script and e.main_script.co ~= nil and e.nav_path and e.nav_path.pi == tu.pick_pi and e.nav_path.ni >= tu.pick_ni then
				if tu.pick_fx then
...
function scripts.mod_freeze.remove(this, store)
	...
	if this._decal_freeze then
		simulation:queue_remove_entity(this._decal_freeze)
	end
...
function scripts.editor_rally_point.remove(this, store)
	if store.entities[this.tower_id] then
		simulation:queue_remove_entity(store.entities[this.tower_id])
	end
...
function scripts.editor_tower.remove(this, store)
	...
	if r then
		simulation:queue_remove_entity(r)
	end
...
function scripts.power_fireball.update(this, store)
	...
	if band(cell_type, TERRAIN_WATER) ~= 0 then
		...
		if this.scorch_earth then
			...
			simulation:queue_insert_entity(scorched)
		end
	else
		if b.hit_decal then
			local decal = E:create_entity(b.hit_decal)
...
scripts.soldier_revive_resist = function(this, store)
	...
	simulation:queue_insert_entity(mod_ban)
...
function scripts.bomb_bouncing.update(this, store)
	...
	while target and not target.health.dead and this.bounce_count > 0 do
		b.damage_factor = b.damage_factor * this.bounce_factor
		b.to.x = target.pos.x
		b.to.y = target.pos.y
		b.from.x = this_pos.x
		b.from.y = this_pos.y
