...
wave_spawn_tsv.name = "wave_spawn_tsv"
wave_spawn_tsv.cmd_fns = {}

function wave_spawn_tsv.cmd_fns.column_names(store, cmd)
...
function wave_spawn_tsv.cmd_fns.flags(store, cmd)
...
function wave_spawn_tsv.cmd_fns.manual_wave(store, cmd)
	log.debug("manual_wave started: %s", cmd.wave_name)
end

function wave_spawn_tsv.cmd_fns.manual_wave_repeat(store, cmd)
...
function wave_spawn_tsv.cmd_fns.call_manual_wave(store, cmd)
	log.debug("call_manual_wave: %s", cmd.value)
	W:start_manual_wave(cmd.value)
end

function wave_spawn_tsv.cmd_fns.wave(store, cmd)
...
function wave_spawn_tsv.cmd_fns.spawn(store, cmd, wave_name)
	local wait_time = hard_wave_wait_time(cmd.wait_time, store.wave_group_number)
...
function wave_spawn_tsv.cmd_fns.event(store, cmd)
...
function wave_spawn_tsv.cmd_fns.signal(store, cmd)
	local wait_time = cmd.wait_time
...
function wave_spawn_tsv.y_run_wave(store, wave_name)
	local cmd_fns = wave_spawn_tsv.cmd_fns
...
function wave_spawn_tsv:init(store)
	...
	store.wave_spawn_co = coroutine.create(wave_spawn_tsv.y_run_wave)
end

function wave_spawn_tsv:on_update(dt, ts, store)
	if store.force_next_wave then
	...
	if W:has_pending_manual_waves() then
		for _, name in pairs(W:list_pending_manual_waves()) do
			...
			store.manual_wave_cos[name] = coroutine.create(wave_spawn_tsv.y_run_wave)
		end
	end
