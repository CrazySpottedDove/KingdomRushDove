...
function perf.set_enabled(enabled)
	if enabled then
		...
		perf.reset = empty_func
		perf.clear = empty_func
		perf.start = empty_func
		perf.stop = empty_func
		perf.wrap = function(func)
			return func
		end
		perf.export_table = function()
			return {}, 0
		end
		perf.tmp_start = empty_func
		perf.tmp_stop = empty_func
		perf.set_frames = empty_func
		perf.set_coros = empty_func
		perf.set_stms = empty_func
	end
end
