...
run_cleanup() {
    cleanup_remote_versions "$QUARK_DIR_ANDROID" '^王国保卫战Dove版-v([0-9]+(?:\.[0-9]+)*)-(?:安卓手机端-低配版|安卓手机端)\.apk$' "$KEEP_LATEST"
...
case "$MODE" in
    ...
    all)
        ...
        mkdir -p ".versions"
        android_apk=".versions/王国保卫战Dove版-v${current_id}-安卓手机端-低配版.apk"
