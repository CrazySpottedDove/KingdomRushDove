...
function scripts.eb_jt.update(this, store)
	...
	while true do
		if this.dying then
			...
			simulation:queue_remove_entity(tap)
...
function scripts.mod_jt_tower.update(this, store)
	...
	U.y_wait_unconditional(store, this.end_delay)
...
function scripts.eb_veznan.update(this, store)
	...
	local function y_block_towers()
		...
		U.y_animation_wait_default(this)
		U.y_wait_unconditional(store, ba.attack_duration - (store.tick_ts - start_ts))
	...
	local function y_portal()
		...
		U.y_animation_wait_default(this)
		U.y_wait_unconditional(store, pa.attack_duration - (store.tick_ts - start_ts))
			...
			if targets then
				for i = 1, #targets do
					...
					if not drained_ids[target.id] then
						local d = E.assign_damage(bor(DAMAGE_DISINTEGRATE, DAMAGE_INSTAKILL), (tonumber(target.health.hp_max) or 0) + 1, this.id, target.id)
	...
	U.y_wait_unconditional(store, fts(24))
	y_taunt()
	U.y_wait_unconditional(store, battle_ts + fts(115) - store.tick_ts)
	...
	simulation:queue_insert_entity(sc)
	U.y_animation_wait_default(this)
	...
	simulation:queue_insert_entity(circle)
	U.y_wait_unconditional(store, fts(65) + 2)
...
function scripts.veznan_souls_aura.update(this, store)
	...
	for i = 1, this.souls.count do
		...
		U.y_wait_unconditional(store, fts(this.souls.delay_frames))
	end
...
function scripts.eb_moloch.update(this, store)
	...
	U.y_wait_unconditional(store, this.stand_up_wait_time)
...
function scripts.eb_myconid.update(this, store)
	...
	local function spawn_mushrooms(count, owner)
		...
		simulation:queue_insert_entity(sp)
	end
			...
			if ready_to_smash() then
				...
				if towers then
					for i = 1, #towers do
						...
						simulation:queue_insert_entity(tm)
					end
				end
				...
				if targets then
					for i = 1, #targets do
						local t = targets[i]
						local d = E.assign_damage(sa.damage_type, math.random(sa.damage_min, sa.damage_max), this.id, t.id)
...
function scripts.eb_efreeti.update(this, store)
	...
	local function do_death()
		...
		simulation:queue_insert_entity(lamp)
...
function scripts.eb_umbra.update(this, store)
	...
	U.y_wait_unconditional(store, fts(113))
	...
	U.y_wait_unconditional(store, 0.6)
	show_taunt(1, 2)
	U.y_wait_unconditional(store, 2)
	show_taunt(2, 2)
	U.y_wait_unconditional(store, 2)
		...
		if is_in_pieces then
			...
			-- block empty
				...
				if store.tick_ts - at.ts > at.cooldown then
					...
					--     simulation:queue_remove_entity(store.entities[last_id])
					--     taunt.last_id = nil
					-- end
					else
						jump_node = home_node
...
function scripts.enemy_umbra_piece.update(this, store)
	...
	U.y_wait_unconditional(store, this.piece_respawn_delay)
...
function scripts.eb_dracula.update(this, store)
	...
	simulation:queue_insert_entity(t)
	U.y_wait_unconditional(store, t.timed.duration + 1)
		...
		if store.tick_ts - last_ts >= a.cycle_time then
			...
			if targets then
				for i = 1, #targets do
					...
					local d = E.assign_damage(a.damage_type, value, this.id, target.id)
...
function scripts.mod_dracula_lifesteal.update(this, store)
	...
	local d = E.assign_damage(target.hero and DAMAGE_TRUE or DAMAGE_INSTAKILL, this.damage, this.id, target.id)
		...
		if this.unit.is_stunned then
			...
			if ready_to_flail() then
				...
				if targets then
					for i = 1, #targets do
						local target = targets[i]
						local val = bit.band(target.vis.flags, F_HERO) ~= 0 and math.random(fa.damage_min_hero, fa.damage_max_hero) or math.random(fa.damage_min, fa.damage_max)
						local d = E.assign_damage(fa.damage_type, val, this.id, target.id)
...
function scripts.eb_drow_queen.update(this, store)
	...
	simulation:queue_insert_entity(d_flying)
	...
	local function y_death()
		...
		simulation:queue_insert_entity(spider)
		...
		simulation:queue_insert_entity(thread)
		...
		simulation:queue_insert_entity(webbing)
		U.y_wait_unconditional(store, fts(13))
		...
		simulation:queue_remove_entity(webbing)
...
function scripts.eb_spider.update(this, store)
	...
	local function y_death()
		...
		simulation:queue_insert_entity(rays)
		U.y_animation_wait_default(this)
	...
	simulation:queue_insert_entity(fx)
	U.y_wait_unconditional(store, fts(45))
		...
		if this.unit.is_stunned and not this.cage_applied then
			...
			if store.tick_ts - spawn_entities_attack.ts >= spawn_entities_attack.cooldown then
				...
				for i = 1, #this.available_nodes do
					...
					simulation:queue_insert_entity(b)
					U.y_wait_unconditional(store, spawn_entities_attack.delay_between)
				end

				U.y_animation_wait_default(this)
				U.y_wait_unconditional(store, spawn_entities_attack.idle_time)
	...
	local function aoe_attack(radius, damage_min, damage_max, damage_type, vis, bans, min_targets)
		...
		if targets and min_targets <= #targets then
			for i = 1, #targets do
				...
				local d = E.assign_damage(damage_type, math.random(dmin, dmax), this.id, target.id)
							...
							elseif ma.type == "area" then
								...
								for i, e in ipairs(targets) do
									...
									-- block empty
										...
										local d = E.assign_damage(ma.damage_type, math.ceil(this.unit.damage_factor * math.random(ma.damage_min, ma.damage_max)), this.id, e.id)
										d.pop = ma.pop
										d.pop_chance = ma.pop_chance
										d.pop_conds = ma.pop_conds
	...
	while true do
		...
		if this.unit.is_stunned and not this.cage_applied then
			...
			if check_life_threshold_stun() then
				...
				U.y_wait_unconditional(store, this.teleport_away_duration)
...
function scripts.boss_navira.update(this, store)
	...
	local function y_on_death()
		...
		if is_tornado then
			S:queue(this.sound_transform_out)
			simulation:queue_remove_entity(this.tornado_aura)
		...
		U.y_wait_unconditional(store, fts(32))
		...
		for i = 1, #delays do
			U.y_wait_unconditional(store, fts(delays[i]))
	...
	local function tornado_in()
		...
		simulation:queue_insert_entity(this.tornado_aura)
	...
	local function tornado_out()
		...
		for i = 1, balls do
			...
			U.y_wait_unconditional(store, fts(10 / balls))
		end
			...
			for _, kill_position in pairs(mask_settings[3]) do
				...
				if soldiers and #soldiers > 0 then
					for _, sold in ipairs(soldiers) do
						local d = E.assign_damage(DAMAGE_INSTAKILL, 99, this.id, sold.id)
						d.pop = nil
	...
	local function break_fn()
		if this.stomp_passive then
			...
			if a.name == "walk" and (a.frame_idx == 16 or a.frame_idx == 36) then
				...
				simulation:queue_insert_entity(step_fx)
				...
				if soldiers then
					for _, soldier in ipairs(soldiers) do
						local d = E.assign_damage(this.stomp_passive.damage_type, U.frandom(this.stomp_passive.damage_min, this.stomp_passive.damage_max), this.id, soldier.id)
...
function scripts.mod_boss_crocs_tower_eat.update(this, store)
	...
	U.y_wait_unconditional(store, this.use_secondary_anim and fts(113) or fts(40))
...
function scripts.mod_croc_boss_evolution_polymorph.insert(this, store)
	...
	if target then
		...
		simulation:queue_insert_entity(entity_poly)
...
function scripts.boss_pig.update(this, store)
	...
	local function y_arrive(to, dest_pi)
		...
		simulation:queue_insert_entity(damage_on_fall)
		...
		simulation:queue_insert_entity(ground_decal)
	...
	local function y_fly(to, speed, dest_pi)
		...
		U.y_wait_unconditional(store, this.fly_time_out)
...
function scripts.boss_machinist.update(this, store)
	...
	U.y_wait_unconditional(store, this.descend_duration)
	...
	simulation:queue_insert_entity(this.burn_aura)
	...
	local function y_do_attack()
		...
		while attacks < this.attacks_count do
			...
			U.y_wait_unconditional(store, a.cooldown - elapsed_time)
...
function scripts.enemy_deformed_grymbeard_clone.update(this, store)
	...
	local function break_fn()
		...
		if this.shield.render.sprites[1].name == "out" and U.animation_finished_default(this.shield) then
	...
	while true do
		if this.health.dead then
			simulation:queue_remove_entity(this.shield)
...
function scripts.boss_grymbeard.update(this, store)
	...
	local function y_on_death()
		...
		U.y_wait_unconditional(store, fts(26))
	...
	while true do
		...
		if this.unit.is_stunned then
			...
			local function check_ranged()
				if not ra.disabled and (ra.requires_magic == false or this.enemy.can_do_magic) and (ra.hold_advance or store.tick_ts - ra.ts > ra.cooldown) then
					...
					if a.use_custom_formation then
						...
						for _, v in pairs(random_formation) do
							...
							if P:is_node_valid(this.nav_path.pi, ni) then
								...
								simulation:queue_insert_entity(obj)
							end
						end
					else
						local sign = 1
				...
				if not soldiers or #soldiers < a.min_targets then
					...
					if soldiers then
						...
						for _, s in ipairs(soldiers) do
							...
							simulation:queue_insert_entity(fx_units)
...
function scripts.aura_boss_spider_queen_spiderweb.update(this, store)
	...
	while true do
		...
		U.y_wait_unconditional(store, this.aura.cycle_time)
	end
end
...
function scripts.mod_boss_spider_queen_tower_debuff.update(this, store)
	...
	simulation:queue_insert_entity(mod_hide)
...
function scripts.bullet_boss_spider_queen_tower_stun.update(this, store)
	...
	if b.particles_name_2 then
		...
		simulation:queue_insert_entity(ps2)
	end
	...
	while true do
		...
		if this.unit.is_stunned then
			...
			if ready_to_fireabsorb() then
				...
				if soldiers then
					for _, s in ipairs(soldiers) do
						local d = E.assign_damage(a_fireabsorb.damage_type, math.random(a_fireabsorb.damage_min, a_fireabsorb.damage_max), this.id, s.id)
