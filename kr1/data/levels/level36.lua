...
if table.contains({"2","9","10","14","16"},h.id) then
...
elseif store.level_mode==GAME_MODE_HEROIC then
for _,h in pairs(self.locations.holders) do
if table.contains({"2","4","9","10","13","15"},h.id) then
LU.insert_tower(store,"tower_holder_blocked_jungle",h.style,h.pos,h.rally_pos,nil,h.id)
else
LU.insert_tower(store,"tower_holder",h.style,h.pos,h.rally_pos,nil,h.id)
end
...
local x
self.nav_mesh={{10,13,x,9},{10,10,1,11},{x,17,6,x},{12,10,11,6},{x,8,14,17},{3,11,9,x},[8]={5,x,13,14},[9]={11,1,x,6},[10]={15,15,1,2},[11]={4,2,9,6},[12]={16,14,4,6},[13]={8,x,1,15},[14]={5,8,15,12},[15]={14,13,10,10},[16]={17,5,12,3},[17]={x,5,16,3}}
...
if store.level_mode==GAME_MODE_CAMPAIGN then
self.sacrifice_delays={nil,nil,52,nil,27,nil,20,nil,nil,72,nil,nil,24,1}
self.volcano_bomb_delay=3
elseif store.level_mode==GAME_MODE_HEROIC then
self.sacrifice_delays={10,10,10,10,10,10}
self.volcano_bomb_delay=3
else
self.sacrifice_delays={[1]=2}
self.volcano_bomb_delay=8
end
local e
local volcano_fires={V.v(867,469),V.v(872,411),V.v(986,414),V.v(976,470)}
...
local smoke_positions={{delay=0.5,pos=V.v(598,457)},{delay=1.1,pos=V.v(621,448)},{delay=1.7,pos=V.v(642,455)}}
local bubble_positions={{delay=0.5,pos=V.v(579,440)},{delay=1.5,pos=V.v(600,430)},{delay=2.5,pos=V.v(599,448)},{delay=2.5,pos=V.v(631,451)},{delay=3,pos=V.v(623,433)},{delay=3.5,pos=V.v(648,431)},{delay=4.5,pos=V.v(656,445)}}
...
::label_5_0::
local bomb_origins={V.v(595,446),V.v(623,459),V.v(634,442)}
local bomb_count=0
