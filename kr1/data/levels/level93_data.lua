return {
	entities_list = {
		{
			template = "background_sounds_blackburn",
			sounds = {"BlackburnGhosts"}
		},
		{
			template = "decal_background",
			pos = {
				x = 1012,
				y = 142
			},
			["render.sprites[1].name"] = "blue_flag",
			["render.sprites[1].z"] = 1400
		},
		{
			template = "decal_background",
			pos = {
				x = 1010,
				y = 235
			},
			["render.sprites[1].flip"] = true,
			["render.sprites[1].name"] = "blue_flag",
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			},
			["render.sprites[1].z"] = 1400
		},
		{
			template = "decal_background",
			pos = {
				x = 512,
				y = 384
			},
			["render.sprites[1].name"] = "stage93",
			["render.sprites[1].z"] = 1000
		},
		{
			template = "decal_background",
			pos = {
				x = 1008,
				y = 504
			},
			["render.sprites[1].flip"] = true,
			["render.sprites[1].name"] = "blue_flag",
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			},
			["render.sprites[1].z"] = 1400
		},
		{
			template = "decal_background",
			pos = {
				x = 1006,
				y = 601
			},
			["render.sprites[1].name"] = "blue_flag",
			["render.sprites[1].z"] = 1400
		},
		{
			template = "decal_bat_flying_1",
			["delayed_play.max_delay"] = 20,
			["delayed_play.min_delay"] = 5,
			pos = {
				x = -7.7400000000000002,
				y = 618.5
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			},
			snapping = "top left"
		},
		{
			template = "decal_bat_flying_2",
			["delayed_play.max_delay"] = 20,
			["delayed_play.min_delay"] = 5,
			pos = {
				x = 502.45999999999998,
				y = 774.13999999999999
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			},
			snapping = "top"
		},
		{
			template = "decal_bat_flying_3",
			["delayed_play.max_delay"] = 20,
			["delayed_play.min_delay"] = 5,
			pos = {
				x = 1031.49,
				y = 454.01999999999998
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			},
			snapping = "top right"
		},
		{
			template = "decal_bat_flying_4",
			["delayed_play.max_delay"] = 20,
			["delayed_play.min_delay"] = 5,
			pos = {
				x = 1028.54,
				y = 116.61
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			},
			snapping = "bottom right"
		},
		{
			template = "decal_bat_flying_5",
			["delayed_play.max_delay"] = 20,
			["delayed_play.min_delay"] = 5,
			pos = {
				x = 687.10000000000002,
				y = -12.1
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			},
			snapping = "bottom"
		},
		{
			template = "decal_bat_flying_6",
			["delayed_play.max_delay"] = 20,
			["delayed_play.min_delay"] = 5,
			pos = {
				x = -12.859999999999999,
				y = 155.90000000000001
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			},
			snapping = "bottom left"
		},
		{
			template = "decal_blackburn_bubble",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 44,
				y = 318
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_bubble",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 180,
				y = 320
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_bubble",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 883,
				y = 335
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_bubble",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 1157,
				y = 352
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_bubble",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 854,
				y = 367
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_bubble",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 854,
				y = 367
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_bubble",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 880,
				y = 398
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_bubble",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 192,
				y = 402
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 73,
				y = 251
			},
			["render.sprites[1].r"] = 0.17453292519943001,
			["render.sprites[1].scale"] = {
				x = 1,
				y = -1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 854,
				y = 262
			},
			["render.sprites[1].r"] = 0.22689280275925999,
			["render.sprites[1].scale"] = {
				x = 1,
				y = -1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 179,
				y = 266
			},
			["render.sprites[1].r"] = 0.22689280275925999,
			["render.sprites[1].scale"] = {
				x = 1,
				y = -1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 328,
				y = 325
			},
			["render.sprites[1].r"] = 0.22689280275925999,
			["render.sprites[1].scale"] = {
				x = 1,
				y = -1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 239,
				y = 332
			},
			["render.sprites[1].r"] = -0.41887902047864001,
			["render.sprites[1].scale"] = {
				x = 1,
				y = -1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 677,
				y = 333
			},
			["render.sprites[1].r"] = -0.41887902047864001,
			["render.sprites[1].scale"] = {
				x = 1,
				y = -1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 774,
				y = 334
			},
			["render.sprites[1].r"] = 0.22689280275925999,
			["render.sprites[1].scale"] = {
				x = 1,
				y = -1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 774,
				y = 334
			},
			["render.sprites[1].r"] = 0.22689280275925999,
			["render.sprites[1].scale"] = {
				x = 1,
				y = -1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 774,
				y = 334
			},
			["render.sprites[1].r"] = 0.22689280275925999,
			["render.sprites[1].scale"] = {
				x = 1,
				y = -1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 1123,
				y = 359
			},
			["render.sprites[1].r"] = -0.48869200000000002,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 363,
				y = 383
			},
			["render.sprites[1].r"] = -0.61086559883619995,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 1104,
				y = 390
			},
			["render.sprites[1].r"] = -2.5830899999999999,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 244,
				y = 392
			},
			["render.sprites[1].r"] = 0.33161255787892002,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 49,
			pos = {
				x = 774,
				y = 398
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 686,
				y = 399
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 58,
				y = 428
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 312,
				y = 428
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 905,
				y = 440
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 806,
				y = 443
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_waves",
			["delayed_play.max_delay"] = 1,
			["delayed_play.min_delay"] = 0,
			pos = {
				x = 199,
				y = 448
			},
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_blackburn_weed",
			pos = {
				x = 1184,
				y = 356
			},
			["render.sprites[1].name"] = "decal_blackburn_weed_idle",
			["render.sprites[1].r"] = -3.1415926535897931,
			["render.sprites[1].scale"] = {
				x = 0.69999999999999996,
				y = 0.69999999999999996
			}
		},
		{
			template = "decal_blackburn_weed",
			pos = {
				x = 1196,
				y = 356
			},
			["render.sprites[1].flip"] = true,
			["render.sprites[1].name"] = "decal_blackburn_weed_idle",
			["render.sprites[1].r"] = 3.1415926535897931,
			["render.sprites[1].scale"] = {
				x = 0.90000000000000002,
				y = 0.90000000000000002
			}
		},
		{
			template = "decal_blackburn_weed",
			pos = {
				x = 1190,
				y = 362
			},
			["render.sprites[1].name"] = "decal_blackburn_weed_idle",
			["render.sprites[1].r"] = 0,
			["render.sprites[1].scale"] = {
				x = 1,
				y = 1
			}
		},
		{
			template = "decal_defend_point",
			["editor.exit_id"] = 2,
			pos = {
				x = 1008,
				y = 179
			}
		},
		{
			template = "decal_defend_point",
			["editor.exit_id"] = 1,
			pos = {
				x = 1008,
				y = 547
			}
		},
		{
			template = "editor_wave_flag",
			["editor.len"] = 40,
			["editor.path_id"] = 6,
			["editor.r"] = 4.7123889803847003,
			pos = {
				x = 492,
				y = 55
			}
		},
		{
			template = "editor_wave_flag",
			["editor.len"] = 40,
			["editor.path_id"] = 8,
			["editor.r"] = 4.7123889803847003,
			pos = {
				x = 532,
				y = 55
			}
		},
		{
			template = "editor_wave_flag",
			["editor.len"] = 240,
			["editor.path_id"] = 4,
			["editor.r"] = 3.1415926535898002,
			pos = {
				x = 40,
				y = 168
			}
		},
		{
			template = "editor_wave_flag",
			["editor.len"] = 240,
			["editor.path_id"] = 3,
			["editor.r"] = 3.1415926535898002,
			pos = {
				x = 40,
				y = 206
			}
		},
		{
			template = "editor_wave_flag",
			["editor.len"] = 240,
			["editor.path_id"] = 2,
			["editor.r"] = 3.1415926535898002,
			pos = {
				x = 43,
				y = 527
			}
		},
		{
			template = "editor_wave_flag",
			["editor.len"] = 240,
			["editor.path_id"] = 1,
			["editor.r"] = 3.1415926535898002,
			pos = {
				x = 44,
				y = 566
			}
		},
		{
			template = "editor_wave_flag",
			["editor.len"] = 30,
			["editor.path_id"] = 5,
			["editor.r"] = 1.5707963267949001,
			pos = {
				x = 491,
				y = 742
			}
		},
		{
			template = "editor_wave_flag",
			["editor.len"] = 30,
			["editor.path_id"] = 9,
			["editor.r"] = 1.5707963267949001,
			pos = {
				x = 528,
				y = 742
			}
		},
		{
			template = "editor_wave_flag",
			["editor.len"] = 30,
			["editor.path_id"] = 7,
			["editor.r"] = 1.5707963267949001,
			pos = {
				x = 162,
				y = 757
			}
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 591,
				y = 96
			},
			["tower.default_rally_pos"] = {
				x = 585,
				y = 174
			},
			["tower.holder_id"] = "17",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "17"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 418,
				y = 109
			},
			["tower.default_rally_pos"] = {
				x = 438,
				y = 170
			},
			["tower.holder_id"] = "14",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "14"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 838,
				y = 112
			},
			["tower.default_rally_pos"] = {
				x = 800,
				y = 179
			},
			["tower.holder_id"] = "22",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "22"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 122,
				y = 117
			},
			["tower.default_rally_pos"] = {
				x = 163,
				y = 179
			},
			["tower.holder_id"] = "5",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "5"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 670,
				y = 119
			},
			["tower.default_rally_pos"] = {
				x = 680,
				y = 177
			},
			["tower.holder_id"] = "18",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "18"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 221,
				y = 120
			},
			["tower.default_rally_pos"] = {
				x = 239,
				y = 178
			},
			["tower.holder_id"] = "12",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "12"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 147,
				y = 218
			},
			["tower.default_rally_pos"] = {
				x = 111,
				y = 177
			},
			["tower.holder_id"] = "10",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "10"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 420,
				y = 219
			},
			["tower.default_rally_pos"] = {
				x = 528,
				y = 200
			},
			["tower.holder_id"] = "13",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "13"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 828,
				y = 219
			},
			["tower.default_rally_pos"] = {
				x = 826,
				y = 184
			},
			["tower.holder_id"] = "11",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "11"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 642,
				y = 221
			},
			["tower.default_rally_pos"] = {
				x = 613,
				y = 185
			},
			["tower.holder_id"] = "16",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "16"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 93,
				y = 475
			},
			["tower.default_rally_pos"] = {
				x = 134,
				y = 531
			},
			["tower.holder_id"] = "15",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "15"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 777,
				y = 478
			},
			["tower.default_rally_pos"] = {
				x = 771,
				y = 533
			},
			["tower.holder_id"] = "4",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "4"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 664,
				y = 479
			},
			["tower.default_rally_pos"] = {
				x = 614,
				y = 538
			},
			["tower.holder_id"] = "21",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "21"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 338,
				y = 480
			},
			["tower.default_rally_pos"] = {
				x = 335,
				y = 537
			},
			["tower.holder_id"] = "7",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "7"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 899,
				y = 583
			},
			["tower.default_rally_pos"] = {
				x = 886,
				y = 535
			},
			["tower.holder_id"] = "3",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "3"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 215,
				y = 591
			},
			["tower.default_rally_pos"] = {
				x = 236,
				y = 548
			},
			["tower.holder_id"] = "2",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "2"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 674,
				y = 592
			},
			["tower.default_rally_pos"] = {
				x = 676,
				y = 538
			},
			["tower.holder_id"] = "1",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "1"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 27,
				y = 594
			},
			["tower.default_rally_pos"] = {
				x = 66,
				y = 543
			},
			["tower.holder_id"] = "8",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "8"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 588,
				y = 603
			},
			["tower.default_rally_pos"] = {
				x = 549,
				y = 560
			},
			["tower.holder_id"] = "6",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "6"
		},
		{
			template = "tower_holder_blackburn",
			["editor.game_mode"] = 0,
			pos = {
				x = 431,
				y = 609
			},
			["tower.default_rally_pos"] = {
				x = 488,
				y = 564
			},
			["tower.holder_id"] = "9",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "9"
		},
		{
			template = "tower_holder_blocked_elemental_wood_enhance",
			["editor.game_mode"] = 0,
			pos = {
				x = 288,
				y = 356
			},
			["tower.default_rally_pos"] = {
				x = 331,
				y = 375
			},
			["tower.holder_id"] = "19",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "19"
		},
		{
			template = "tower_holder_blocked_elemental_wood_enhance",
			["editor.game_mode"] = 0,
			pos = {
				x = 716,
				y = 360
			},
			["tower.default_rally_pos"] = {
				x = 762,
				y = 368
			},
			["tower.holder_id"] = "20",
			["tower.terrain_style"] = 15,
			["ui.nav_mesh_id"] = "20"
		}
	},
	invalid_path_ranges = {},
	level_mode_overrides = {
		[3] = {
			locked_hero = false,
			locked_towers = {"tower_build_archer", "tower_build_mage"},
			max_upgrade_level = 5
		}
	},
	level_terrain_type = 12,
	locked_hero = false,
	max_upgrade_level = 5,
	nav_mesh = {
		{nil, 2, 12, 4},
		{1, nil, 7, 12},
		{nil, 4, 4, 6},
		{3, 1, 11, 6},
		{13, 10},
		{3, 11, 14},
		{2, nil, 8, 9},
		{7, nil, 15, 10},
		{12, 7, 10, 13},
		{9, 8, 15, 5},
		{4, 12, 14, 6},
		{1, 2, 9, 11},
		{14, 9, 10, 5},
		{11, 9, 13, 6},
		{8, 8, nil, 10},
		{},
		{},
		{},
		{},
		{}
	},
	required_exoskeletons = {
		"stage_31_fire_ADef",
		"stage_31_fire_BDef",
		"stage_31_fire_CDef",
		"stage_31_forest_01Def",
		"stage_31_forest_02Def",
		"stage_31_forest_03Def",
		"stage_31_fireball_ADef",
		"stage_31_fireball_BDef",
		"stage_31_fireball_CDef",
		"stage_31_waterfall_layer_1Def",
		"stage_31_waterfall_layer_2Def",
		"stage_31_waterfall_layer_3Def",
		"stage_31_waterfall_layer_4Def",
		"stage_31_waterfall_layer_5Def",
		"stage_31_waterfall_layer_6Def",
		"stage_31_waterfall_layer_7Def",
		"fuente_unitDef",
		"water_splash_unitDef",
		"charco_unitDef",
		"water_cracksDef",
		"stage31_wood_holder_cuernosDef",
		"stage31_wood_holder_dragon_rootDef",
		"stage31_wood_holder_dragonDef",
		"stage31_wood_holder_gradienteDef",
		"stage31_wood_holder_habilidad_1Def",
		"stage31_wood_holder_jarraDef",
		"stage31_wood_holder_jarrahojasDef",
		"stage31_wood_holder_rayo_explosionDef",
		"stage31_wood_holder_rayoDef",
		"stage31_wood_holder_root1Def",
		"stage31_wood_holder_root2Def",
		"stage31_wood_holder_root3Def",
		"stage31_wood_holder_root4Def",
		"stage31_wood_holder_animations_parcheDef",
		"stage_31_shadowDef",
		"ash_spiritDef",
		"stage_31_oogwayDef"
	},
	required_sounds = {"music_stage93", "BlackburnSounds", "SaurianKingBoss"},
	required_textures = {
		"go_enemies_blackburn",
		"go_enemies_halloween",
		"go_stages_blackburn",
		"go_stage24",
		"go_stage26",
		"go_stage48",
		"go_stage93_bg",
		"go_wukong_elemental_holders",
		"go_hero_alleria",
		"go_hero_10yr",
		"go_hero_gerald"
	}
}
