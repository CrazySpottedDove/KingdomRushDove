...
local function enemy_pick_target_and_do_ranged_attack(store, this, attack)
	...
	local target = U.find_nearest_soldier(entities, this.pos, attack.min_range, attack.max_range, attack.vis_flags, attack.vis_bans)
	...
	while true do
		...
		if this.unit.is_stunned then
			...
			if multishot_ready() then
				...
				local target = U.find_nearest_soldier(entities, this.pos, multishot.min_range, multishot.max_range, multishot.vis_flags, multishot.vis_bans)
...
tt.health.on_damage = function(this, store, damage)
	...
	if this.unit.is_stuuned or this.health.dead or bda.in_progress or band(damage.damage_type, DAMAGE_ALL_TYPES, bnot(bor(DAMAGE_PHYSICAL, DAMAGE_MAGICAL, (DAMAGE_MIXED or 0)))) ~= 0 or band(damage.damage_type, DAMAGE_NO_DODGE) ~= 0 or this.dodge.chance < math.random() or (not this.enemy.can_do_magic) then
		return true
	end
...
tt.main_script.update = function(this, store)
	...
	local function range_ready()
		...
		local targets = U.find_soldiers_in_range(entities, this.pos, this.ranged.attacks[2].min_range, this.ranged.attacks[2].max_range, this.ranged.attacks[2].vis_flags, this.ranged.attacks[2].vis_bans)
	...
	while true do
		...
		if this.unit.is_stunned then
			...
			if ea_ready() then
				local targets = U.find_soldiers_in_range(entities, this.pos, 0, ea.max_range, ea.vis_flags, ea.vis_bans, function(s)
			...
			if not SU.y_enemy_mixed_walk_melee_ranged(store, this, false, break_from_walk, break_from_melee, break_from_range) then
				goto label_56_3
			end
		end
	...
	while true do
		...
		if this.unit.is_stunned then
			...
			if this.is_buffed and store.tick_ts - a.ts >= a.cooldown then
				local soldiers = U.find_soldiers_in_range(soldier_entities, this.pos, a.min_range, a.max_range, a.vis_flags, a.vis_bans)
