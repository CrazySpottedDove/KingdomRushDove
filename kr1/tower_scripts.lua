...
function scripts.soldier_templar.side_effect_1(this, store, damage, target)
	...
	if target then
		local d = E.assign_damage(DAMAGE_TRUE, math.ceil(this.health.hp_max * this.powers.extralife.damage_factor * this.powers.extralife.level), this.id, target.id)
...
function scripts.soldier_templar.side_effect_2(this, store, damage, target)
	...
	if target then
		target.health.damage_factor = target.health.damage_factor * this.powers.blood.vulnerability_factor

		local d = E.assign_damage(DAMAGE_TRUE, math.ceil(this.health.hp_max * this.powers.extralife.damage_factor_blood * this.powers.extralife.level), this.id, target.id)
		...
		while true do
			...
			-- block empty
				...
				if a then
					...
					local an, af = U.animation_name_facing_point_use_offset(this, a.animation, enemy.pos, shooter_sid, start_offset)

					U.animation_start_once_specific(this, an, af, store.tick_ts, shooter_sid)
...
scripts.tower_ranger = {
	update = function(this, store)
		...
		local function add_attacked_enemy(enemy)
			...
			table.insert(attacked_enemies_ts, store.tick_ts + aa.shoot_time)
		end
...
scripts.tower_musketeer = {
	update = function(this, store)
		...
		local function shot_animation(attack, shooter_idx, enemy, animation)
			local ssid = shooter_sids[shooter_idx]
			local an, af, ai = U.animation_name_facing_point_use_offset(this, animation or attack.animation, enemy.pos, ssid, sprites[ssid].offset)
		...
		local function shot_bullet(attack, shooter_idx, ani_idx, enemy, level)
			...
			if attack == asn then
				...
				if band(enemy.vis.flags, F_BOSS) ~= 0 then
					bl.damage_max = bl.damage_max * (pow_sn.boss_damage_base + pow_sn.boss_damage_inc * pow_sn.level)
					bl.damage_min = bl.damage_min * (pow_sn.boss_damage_base + pow_sn.boss_damage_inc * pow_sn.level)
				else
			...
			if tw.blocked then
				...
				if pow_sn.level > 0 then
					for _, ax in ipairs({asi, asn}) do
						if (ax.chance == 1 or random() < ax.chance) and U.tower_ready_to_use_power(pow_sn, ax, store, tw) then
							...
							local _, _, ai = shot_animation(ax, shooter_idx, enemy)
				...
				if U.tower_ready_to_use_power(pow_sh, ash, store, tw) then
					...
					if not enemy then
						...
						local _, _, ai = shot_animation(ash, shooter_idx, enemy)
			...
			if tw.blocked then
				...
				if U.tower_ready_to_use_power(pow_m, ma, store, tw) then
					...
					if not enemy then
						...
						local an, af = U.animation_name_facing_point_use_offset(this, "multishot_start", enemy.pos, shooter_sid, start_offset)
						...
						an, af = U.animation_name_facing_point_use_offset(this, "multishot_loop", enemy.pos, shooter_sid, start_offset)
						...
						local an, af = U.animation_name_facing_point_use_offset(this, "multishot_end", last_enemy.pos, shooter_sid, start_offset)
				...
				if ready_to_attack(aa, store, tw.cooldown_factor) then
					...
					if not enemy then
						...
						local an, af = U.animation_name_facing_point_use_offset(this, "shoot", enemy.pos, shooter_sid, start_offset)
			...
			-- block empty
				...
				if ready_to_attack(aa, store, tw.cooldown_factor) then
					...
					if not enemy then
						...
						local shooter_sid = shooter_sprite_ids[shooter_idx]
						local start_offset = aa.bullet_start_offset[shooter_idx]
						local an, af = U.animation_name_facing_point_use_offset(this, aa.animation, enemy.pos, shooter_sid, start_offset)
...
function scripts.pirate_watchtower_parrot.update(this, store)
	...
	while true do
		...
		if store.tick_ts - ca.ts > ca.cooldown * this.owner.tower.cooldown_factor and not this.owner.tower.blocked then
...
scripts.tower_pirate_watchtower = {
	remove = function(this, store)
		for _, parrot in ipairs(this.parrots) do
			parrot.owner = nil
		end
					...
					if not enemy then
						...
						for i = 1, #shooter_sids do
							...
							if enemy.health and enemy.health.magic_armor > 0 then
								sa.ts = sa.ts - this.powers.burst.magic_armor_hit_cooldown_reduction
							end
	...
	update = function(this, store)
		...
		local function y_do_shot(attack, enemy, level)
			...
			if b.template_name == "arrow_silver_sentence" then
				bl.damage_factor = bl.damage_factor * (pow_s.damage_factor_base + pow_s.damage_factor_inc * pow_s.level)

				if band(enemy.vis.flags, F_BOSS) ~= 0 then
					bl.damage_factor = bl.damage_factor / pow_s.boss_damage_factor
				end
			end
		...
		while true do
			if tw.blocked then
				...
				if pow_m.changed then
					...
					if pow_m.level == 1 then
						this.attacks.list[3].ts = store.tick_ts
					end
				...
				if ready_to_attack(aa, store, tw.cooldown_factor) then
					...
					if enemies then
						...
						if pow_s.level > 0 then
							...
							if mark then
								chance = chance * pow_m.sentence_chance_factor
							end
						end
		...
		while true do
			...
			-- block empty
				...
				if pow_e.changed then
					...
					ea.cooldown = pow_e.cooldowns[pow_e.level]
				end

				if pow_w.changed then
					pow_w.changed = nil

					if pow_w.level == 1 then
						wa.ts = store.tick_ts
					end
				end
...
scripts.tower_arcane_wizard = {
	...
	update = function(this, store)
		...
		local function start_animations(attack, enemy)
			...
			local an, af = animation_name_facing_point(this, attack.animation, enemy.pos, shooter_sid, soffset)
				...
				for i, aa in ipairs(attacks) do
					...
					if (pow and U.tower_ready_to_use_power(pow, aa, store, this.tower)) or (not pow and ready_to_attack(aa, store, this.tower.cooldown_factor)) then
						...
						if not enemy then
							...
							local soffset = this.render.sprites[shooter_sid].offset
							local an, _, ai = animation_name_facing_point(this, aa.animation, enemy.pos, shooter_sid, soffset)
	...
	update = function(this, store)
		...
		local function prepare_bullet(start_offset, i)
			...
			if pow_b.level > 0 and random() < ba.payload_chance then
				...
				blast.bullet.source_id = this.id
			...
			if tw.blocked then
				...
				if U.tower_ready_to_use_power(pow_t, ta, store, tw) then
					...
					if not target then
						...
						twister.aura.source_id = this.id
	...
	insert = function(this, store)
		...
		for _, a in pairs(this.auras.list) do
			if a.cooldown == 0 then
				...
				e.aura.damage_factor = this.tower.damage_factor
				...
				if U.tower_ready_to_use_power(pow_p, pa, store, this.tower) then
					...
					if enemy then
						...
						for i = 1, count do
							...
							b.aura.damage_factor = this.tower.damage_factor
		...
		while true do
			do
				...
				for _, enemy in ipairs(enemies) do
					...
					local enemy_gold = enemy and enemy.enemy and enemy.enemy.gold or nil
					local enemy_is_boss = enemy and enemy.vis and band(enemy.vis.flags or 0, F_BOSS) ~= 0
					...
					if pow_g.level > 0 then
						if exact_damage >= enemy.health.hp then
							if enemy_gold and enemy_gold > 0 and not enemy_is_boss then
					...
					if pow_c.level > 0 then
						-- 对 Boss 或“预测击杀但实际不死”的对象不计充能，避免异常加速。
						if exact_damage >= enemy.health.hp and not enemy_is_boss then
...
function scripts.decal_pixie.update(this, store)
	...
	while true do
		...
		elseif store.tick_ts - iflip.ts > iflip.cooldown then
			U.animation_start_default(this, table.random(iflip.animations), random() < 0.5, store.tick_ts, iflip.loop)
					...
					if std_ready then
						...
						if trigger_enemy then
							...
							if enemies then
								...
								dvalue = this.tower.damage_factor * dvalue * (lava_ready and (pow_l.level * pow_l.damage_mult_inc + 1) or 1)

								local u = UP:get_upgrade("engineer_magic_dust")

								for _, enemy in ipairs(enemies) do
									local d = E.assign_damage(aa.damage_type, dvalue, this.id, enemy.id)
		...
		local function do_attack(at)
			...
			if target then
				...
				if b.bullet.hit_peyload then
					...
					pl.aura.damage_factor = this.tower.damage_factor
					...
					if U.has_enough_enemies_in_range(store, tpos(this), 0, ca.range, ca.vis_flags, ca.vis_bans, nil, ca.min_count) then
						...
						if targets then
							for i, target in ipairs(targets) do
								local d = E.assign_damage(ca.damage_type, pow_c.damage_values[pow_c.level] * this.tower.damage_factor, this.id, target.id)
...
scripts.mod_tesla_overcharge = {
	insert = function(this, store)
		if scripts.mod_track_target.insert(this, store) then
			...
			if math.random() < this.stun_chance and band(target.vis.bans, F_STUN) == 0 and band(target.vis.flags, F_BOSS) == 0 then
				SU.stun_inc(target)
	...
	while true do
		if tw.blocked then
			...
			if ready_to_attack(ab, store, tw.cooldown_factor) then
				local trigger, enemies = U.find_foremost_enemy_in_range_filter_off(tpos, a.range, ab.node_prediction, ab.vis_flags, ab.vis_bans)
	...
	while true do
		if this.tower.blocked then
			...
			if store.tick_ts - last_ts > this.tower.long_idle_cooldown then
				...
				animation_start(this, an, af, store.tick_ts, true, this.render.sid_archer)
...
function scripts.mod_tower_dark_elf_big_target.update(this, store)
	...
	U.animation_start_default(this, "run", nil, store.tick_ts)
		...
		if store.tick_ts - check_soldiers_ts > this.check_soldiers_cooldown and not this.tower_ref.blocked then
			for i = 1, b.max_soldiers do
				...
				if not s or s.health.dead and not store.entities[s.id] then
					...
					U.animation_start_default(this, "idle", false, store.tick_ts)
...
function scripts.mod_tower_dark_elf_skill_buff.remove(this, store)
	...
	if not target or not target.health or not target.health.dead then
		return true
	end
...
function scripts.bullet_tower_dark_elf_skill_buff.update(this, store)
	...
	if b.store and not b.target_id then
		...
		U.animation_start_default(this, "soul_travel", nil, store.tick_ts, s.loop)
...
function scripts.soldier_tower_demon_pit.update(this, store)
	...
	if tower and tower.powers.master_exploders.level > 0 then
	...
	local function explosion(r, damage_min, damage_max, dty)
		...
		if targets then
			for _, target in ipairs(targets) do
				local d = E.assign_damage(dty, (u and damage_max or random(damage_min, damage_max)) * factor, this.id, target.id)
...
function scripts.big_guy_tower_demon_pit.update(this, store)
	...
	local function explosion(r, damage, dty)
		...
		if targets then
			for _, target in ipairs(targets) do
				local d = E.assign_damage(dty, damage, this.id, target.id)
	...
	while true do
		...
		for i, panda in pairs(this.pandas) do
			...
			if spr.name == "spell" and animation_finished(this, panda.render) then
				...
				animation_start(this, an, af, store.tick_ts, true, panda.render)
		...
		for _, panda in pairs(this.pandas) do
			...
			if cfg and store.tick_ts >= cfg.shoot_ts then
				...
				if enemy then
					...
					local _, af = animation_name_facing_point(this, "shoot", enemy.pos, cfg.shooter_sid, cfg.start_offset)
...
function scripts.tower_pandas_ray.update(this, store)
	...
	local function update_sprite()
		...
		s.scale.x = (V.dist(dest.x, dest.y, this.pos.x, this.pos.y) + dist_offset) / image_width
	end
	...
	if b.hit_payload then
		...
		if hp.aura then
			...
			hp.aura.damage_factor = this.bullet.damage_factor
		...
		if this.unit.is_stunned then
								...
								if affected then
									for _, enemy in ipairs(affected) do
										if enemy.health and not enemy.health.dead then
											...
											local d = E.assign_damage(a_i.damage_type, random(dmin, dmax) * this.unit.damage_factor, this.id, enemy.id)
			...
			if can_teleport() then
				...
				if not target or not targets or #targets < 1 then
					...
					U.animation_start_default(this, a_i.animation, nil, store.tick_ts, false)
					...
					if not target or #targets < 1 then
						...
						for i = 1, num_targets do
							local t = targets[i]
							local dmin, dmax = a_i.damage_min, a_i.damage_max
							local d = E.assign_damage(a_i.damage_type, random(dmin, dmax) * this.unit.damage_factor, this.id, t.id)
...
function scripts.tower_ray.update(this, store)
		...
		while store.tick_ts - t0 < duration do
			if ray_target_invalid(store, eid) then
				...
				if enemy then
					...
					local an, af = animation_name_facing_point(this, aa.animation_start, enemy.pos, this.render.sid_mage, this.mage_offset)
	...
	do
		...
		local an, af = animation_name_facing_point(this, "idle", a._last_target_pos, this.render.sid_mage, soffset)
	...
	while true do
		...
		-- block empty
				...
				if not aa.disabled and ready_to_attack(aa, store, this.tower.cooldown_factor) then
					if aa == as then
						...
						if not enemy then
							...
							do
								local an, af = animation_name_facing_point(this, aa.animation_end, a._last_target_pos, this.render.sid_mage, this.mage_offset)
			...
			if store.tick_ts - idle_ts > this.tower.long_idle_cooldown then
				local an, af = animation_name_facing_point(this, "idle", this.tower.long_idle_pos, this.render.sid_mage, this.mage_offset)
...
function scripts.bullet_tower_ray.update(this, store)
	...
	b.cooldown_factor = tonumber(b.cooldown_factor) or tonumber(tower and tower.tower and tower.tower.cooldown_factor) or 1
	...
	local start_ts = store.tick_ts
	local pending_chain = this.chain_pos and this.max_enemies and this.chain_pos < this.max_enemies
	...
	while store.tick_ts - start_ts < ray_duration and target and not this.force_stop_ray and source do
		...
		if pending_chain and tower and tower.chain_targets and store.tick_ts - start_ts > this.chain_delay then
		...
		do
			local nic = this.next_in_chain
			local nic_s = nic and nic.render and nic.render.sprites and nic.render.sprites[1]

			if chained_next_ray and (not nic or not nic_s or nic_s.hidden) then
				pending_chain = true
				chained_next_ray = false
			end
		end

		if this.chain_pos and this.chain_pos > 1 and source and source.unit then
		...
		if store.tick_ts - start_ts > ray_duration - fts(7) and this.render.sprites[1].name ~= "fade" then
			U.animation_start_default(this, "fade", nil, store.tick_ts)
		end
		...
		if target and this.chain_pos and this.chain_pos > 1 and this.chain_range_to_stay and V.dist2(this.pos.x, this.pos.y, target.pos.x, target.pos.y) > this.chain_range_to_stay * this.chain_range_to_stay then
			break
		end

		if target and target.vis and band(target.vis.bans, this.vis_flags) ~= 0 then
			this.force_stop_ray = true
	...
	if tower and tower.chain_targets then
		...
		-- block empty
			...
			if targets then
				for i, target in ipairs(targets) do
					...
					local d = E.assign_damage(this.aura.damage_type, random(dmin, dmax) * this.aura.damage_factor, this.id, target.id)
	...
	while true do
		...
		if store.tick_ts - ca.ts > ca.cooldown and distance_from_target < this.min_distance_to_attack then
			...
			if target and target.health.dead then
				...
				local d = E.assign_damage(ca.damage_type, random(ca.damage_min[this.level], ca.damage_max[this.level]), this.id, target.id)
...
function scripts.tower_royal_archers_pow_rapacious_hunter_tamer_mark_mod.update(this, store)
	...
	while true do
		...
		if not target or not target.health or target.health.dead or m.duration >= 0 and store.tick_ts - m.ts > m.duration then
...
function scripts.tower_arcane_wizard5.update(this, store)
	...
	do
		local soffset = this.shooter_offset
		local an = animation_name_facing_point(this, "idle", a._last_target_pos, shooter_sid, soffset)
		...
		if this.tower.blocked then
			...
			if U.tower_ready_to_use_power(pow_d, ad, store, tw) then
				...
				if not enemy then
					...
					local soffset = this.shooter_offset
					local an, _, ai = animation_name_facing_point(this, ad.animation, enemy.pos, shooter_sid, soffset)
			...
			if ready_to_attack(ar, store, tw.cooldown_factor) then
				...
				if not enemy then
					...
					local soffset = this.shooter_offset
					local an, _, ai = animation_name_facing_point(this, ar.animation, enemy.pos, shooter_sid, soffset)
			...
			if store.tick_ts - ar.ts > this.tower.long_idle_cooldown then
				local soffset = this.shooter_offset
				local an, _ = animation_name_facing_point(this, "idle", this.tower.long_idle_pos, shooter_sid, soffset)
...
local function tower_rocket_gunners_phosphoric_area_damage(soldier, store, target)
	...
	for _, enemy in ipairs(enemies) do
		...
		if upg then
			...
			dvalue = math.floor(dmax - (dmax - dmin) * dist_factor)
		end

		dvalue = dvalue * soldier.unit.damage_factor

		local d = E.assign_damage(attack.damage_type, dvalue, soldier.id, enemy.id)
...
function scripts.soldier_tower_rocket_gunners.update(this, store)
	...
	if not this.spawned_from_tower then
		...
		if this.current_mode == MODE_FLY then
			...
			U.animation_start_default(this, "idle_floor", nil, store.tick_ts, true)
...
function scripts.bullet_soldier_tower_rocket_gunners_sting_missiles.update(this, store)
	...
	if target and not target.health.dead then
		local d = E.assign_damage(DAMAGE_INSTAKILL, 10, this.id, target.id)
					...
					if store.tick_ts - fire_cycle_ts >= attack_basic.cycle_time * tw.cooldown_factor then
						...
						if aura_targets then
							for _, aura_target in ipairs(aura_targets) do
								...
								if up then
									dvalue = attack_basic.damage_max * tw.damage_factor
								else
									dvalue = (random(attack_basic.damage_min, attack_basic.damage_max)) * tw.damage_factor
								end

								local d = E.assign_damage(attack_basic.damage_type, dvalue, this.id, aura_target.id)
								local u = UP:get_upgrade("engineer_magic_dust")
...
function scripts.controller_tower_flamespitter_column.update(this, store)
	...
	if enemies and #enemies > 0 then
		for _, enemy in ipairs(enemies) do
			local d = E.assign_damage(this.damage_in_type, random(this.damage_in_min, this.damage_in_max) * this.damage_factor, this.source_id, enemy.id)
	...
	if enemies and #enemies > 0 then
		for _, enemy in ipairs(enemies) do
			local d = E.assign_damage(this.damage_out_type, random(this.damage_out_min, this.damage_out_max) * this.damage_factor, this.source_id, enemy.id)
			...
			if ready_to_attack(aa, store, tw.cooldown_factor) then
				...
				for i = 1, aa.burst_count do
					...
					if pow_s.level > 0 and i == aa.burst_count then
						...
						U.animation_start(this, a_name, a_flip, store.tick_ts, false, this.render.sid_tower_top)
						U.y_wait_unconditional(store, fts(6) * tw.cooldown_factor)
				...
				U.y_wait_unconditional(store, fts(15) * tw.cooldown_factor)
				...
				U.y_animation_play(this, a_name, a_flip, store.tick_ts, false, this.render.sid_tower_top)
				U.y_wait_unconditional(store, fts(4) * tw.cooldown_factor)
...
function scripts.bullet_tower_ballista_skill_bomb.update(this, store)
	...
	if enemies then
		for _, enemy in ipairs(enemies) do
			...
			dvalue = math.ceil(b.damage_factor * dvalue)

			local d = E.assign_damage(b.damage_type, dvalue, this.id, enemy.id)
...
function scripts.aura_bullet_tower_ballista_skill_bomb.update(this, store)
	...
	U.animation_start_specific(this, "in", false, store.tick_ts, false, 1)

	while true do
		if U.animation_finished_default(this) and this.render.sprites[1].name ~= "idle" then
			U.animation_start_specific(this, "idle", false, store.tick_ts, true, 1)
		end
			...
			if targets then
				if this.render.sprites[1].name == "idle" then
					...
					if #moving_targets > 0 then
						U.animation_start_specific(this, "ability1", false, store.tick_ts, false, 1)
		...
		if this.tower.blocked then
			...
			if U.tower_ready_to_use_power(pow_b, bba, store, tw) then
				...
				if not targets or #targets < bba.min_targets then
					...
					U.y_wait_unconditional(store, bba.shoot_time)

					local trigger_pos = pred_pos
					local enemy, _, pred_pos = U.find_foremost_enemy_with_max_coverage_in_range_filter_off(tpos, a.range, bba.node_prediction, bba.vis_flags, bba.vis_bans, E:get_template("bullet_tower_barrel_skill_barrel").bullet.damage_radius)
			...
			if U.tower_ready_to_use_power(pow_w, wa, store, tw) then
				for i = 1, b.max_soldiers do
					...
					if not s or s.health.dead and not store.entities[s.id] then
						...
						U.y_wait_unconditional(store, wa.drunk_man_spawn_delay)
...
function scripts.aura_bullet_tower_barrel_skill_barrel.update(this, store)
	...
	if targets then
		for i, target in ipairs(targets) do
			...
			dvalue = math.ceil(dvalue * a.damage_factor)

			local d = E.assign_damage(this.explosion_damage_type, dvalue, this.id, target.id)
...
function scripts.controller_soldier_tower_barrel_skill_warrior_spawn.update(this, store)
	U.animation_start_specific(this, "idle", false, store.tick_ts, false, 1)
	U.y_wait_unconditional(store, this.berzerker_spawn_delay)
...
function scripts.tower_hermit_toad.update(this, store)
	...
	local function change_mode(new_mode, with_animation)
		...
		if not is_init then
			...
			U.y_wait_unconditional(store, fts(9 * tw.cooldown_factor))
			create_splash_fx()
			U.y_wait_unconditional(store, fts(24 * tw.cooldown_factor))
		else
	...
	local function y_toad_flip(af)
		...
		pause_pipe_ps()
		get_mode_anim(this.toad_flip_anims)
		...
		if tw.blocked then
			...
			if U.tower_ready_to_use_power(pow_instakill, attack_instakill, store, tw) then
				...
				if (not target) or (U.has_modifiers(store, target, attack.mark_mod)) then
					...
					U.y_wait_unconditional(store, fts(1 * tw.cooldown_factor))
					...
					if target then
						...
						for _, s in pairs(target.render.sprites) do
							if s.animated then
								table.insert(entity_frame_names, A:frame_name(s.prefix and (s.prefix .. "_" .. s.name) or s.name, s.frame_idx))
							else
								table.insert(entity_frame_names, s.name)
							end
						end
						...
						do
							local bs = bullet.render.sprites[1]

							while not eaten do
								if bs.frame_idx == 11 or bs.frame_idx == 12 or not store.entities[bullet.id] then
			...
			if U.tower_ready_to_use_power(pow_jump, attack_jump, store, tw) then
				...
				if not targets or #targets < attack.min_targets then
					...
					local an, af = U.animation_name_facing_point(this, get_mode_anim(attack.animation_start), pred_pos, 3)
					...
					if targets then
						for _, t in ipairs(targets) do
							...
							local dvalue = math.floor(attack.damage_max - (attack.damage_max - attack.damage_min) * dist_factor)

							dvalue = math.ceil(this.tower.damage_factor * dvalue)

							local d = E.assign_damage(attack.damage_type, dvalue, this.id, t.id)
	...
	while true do
		...
		-- block empty
			...
			if can_crystalize() then
				...
				U.y_wait_unconditional(store, a_crystalize.cast_time)
			...
			if can_spike_burst() then
				...
				U.y_wait_unconditional(store, a_burst.cast_time)
...
function scripts.tower_sparking_geode_ray.update(this, store)
	...
	this.seen_targets = this.seen_targets or {}
	...
	local function y_soldier_new_rally_break_attack(store, this, first_walk)
		...
		if r.new then
			...
			if V.dist2(r.center.x, r.center.y, old_center.x, old_center.y) < this.max_dist_walk * this.max_dist_walk or first_walk then
				...
				local _, af = U.animation_name_facing_point(this, "jump_in", this.motion.dest)
	...
	while true do
		...
		if pow_e.changed then
			...
			if this._aura_extra_damage == nil then
				...
				e.aura.damage_factor = E:get_template("tower_ghost_lvl4").tower.damage_factor
		...
		if this.unit.is_stunned then
			...
			do
				...
				if not a.disabled and this.health.hp <= this.health.hp_max * a.lost_health and store.tick_ts - a.ts > a.cooldown then
					U.animation_start_default(this, a.animation .. "_start", nil, store.tick_ts)
					S:queue(a.sound)
					...
					-- block empty
						...
						-- block empty
							...
							U.animation_start_default(this, a.animation .. "_loop", nil, store.tick_ts, true)

							if SU.y_soldier_wait(store, this, a.duration - (store.tick_ts - a.ts)) then
							-- block empty
							else
								U.animation_start_default(this, a.animation .. "_end", nil, store.tick_ts)
						...
						if enemies then
							for i = 1, #enemies do
								local damage = E.assign_damage(a.beat_back_damage_type, a.beat_back_damage_value * this.unit.damage_factor, this.id, enemies[i].id)
...
function scripts.decal_tower_arborean_emissary_gift_of_nature_wisp.update(this, store)
	...
	U.y_wait_unconditional(store, fts(this.wisp_order) * 3)
	...
	while true do
		...
		if store.tick_ts - a.ts > a.cooldown then
			...
			-- block empty
				...
				U.animation_start_default(this, a.animation, flip_x, store.tick_ts, false)
...
function scripts.tower_dragons.update(this, store)
	...
	local points, total_enemies, count_targets, bullet, anim_idx, offset_x, offset_y
	local a = this.attacks
	local a_basic = this.attacks.list[1]
		...
		elseif can_sleep() then
			...
			if can_massive_fear() then
				...
				if not enemies or #enemies < a_massive_fear.min_targets then
					...
					U.y_wait_unconditional(store, a_massive_fear.cast_time)
					...
					if count_targets < a_massive_fear.max_targets then
						U.y_wait_unconditional(store, fts(30) * tw.cooldown_factor)
...
function scripts.bullet_tower_dragons_dragon_split.update(this, store)
	...
	if target and not target.health.dead then
		...
		if next_target then
			...
			if next_target.health and not next_target.health.dead then
				do_hit(next_target)
			else
		...
		while ca.active do
			if ca.target_id ~= nil and store.tick_ts - ca.ts > ca.cooldown * this.owner.tower.cooldown_factor then
				...
				-- block empty
					...
					U.y_wait_unconditional(store, ca.shoot_time)
	...
	while true do
		if this.tower.blocked then
					...
					if enemy then
						...
						if not enemy.health.dead then
							...
							if (enemy.ranged or enemy.timed_attacks or enemy.auras or enemy.death_spawns) then
								as.ts = as.ts - as.cooldown * pow_s.caster_cooldown_refund * this.tower.cooldown_factor
							end
						else
							as.ts = as.ts + 5
						end
			...
			if U.tower_ready_to_use_power(pow_m, am, store, this.tower) then
				...
				if enemies then
					for i = 1, #enemies do
						if enemies[i].health.hp >= pow_m.hp_threshold then
							enemy = enemies[i]
...
function scripts.tower_rotten_forest.update(this, store, script)
	...
	local last_active_path_id_sum = P:active_path_id_sum()
	...
	while true do
		if this.tower.blocked then
			...
			-- 同步范围
			if last_range ~= a.range or last_active_path_id_sum ~= P:active_path_id_sum() then
				last_range = a.range
				last_active_path_id_sum = P:active_path_id_sum()
...
function scripts.decal_rotten_forest_smoke.update(this, store)
	U.animation_start_default(this, "intro", nil, store.tick_ts, false)
...
function scripts.tower_infernal_mage.update(this, store)
	...
	local function do_shoot_animation(aa, enemy)
		...
		local soffset = this.render.sprites[shooter_sid].offset
		local an, af = U.animation_name_facing_point(this, aa.animation, enemy.pos, shooter_sid, soffset)
...
function scripts.aura_lava_fissure.update(this, store)
	...
	if targets then
		for _, e in ipairs(targets) do
			local d = E.assign_damage(a.damage_type, math.random(a.damage_min, a.damage_max) * a.damage_factor, this.id, e.id)
			...
			if ready_to_attack(ba, store, this.tower.cooldown_factor) then
				...
				if not target then
					...
					if shot_target then
						...
						b.pos:copy(shot_target.pos)
						b.bullet.from:copy(b.pos)
						b.bullet.to:copy(b.pos)
...
function scripts.bullet_orc_shaman_shock.update(this, store)
	...
	if enemies then
		for i = 1, #enemies do
			local enemy = enemies[i]
			local d = E.assign_damage(b.damage_type, math.ceil(U.frandom(dmin, dmax) * b.damage_factor), this.id, enemy.id)
...
function scripts.bolt_orc_shaman.update(this, store)
	...
	if target and not target.health.dead then
		...
		if mods then
			for i = 1, #mods do
				...
				if band(target.vis.bans, m.modifier.vis_flags) == 0 and band(target.vis.flags, m.modifier.vis_bans) == 0 then
					m.modifier.target_id = target.id
					m.modifier.source_id = this.id
					m.modifier.level = b.level
					m.modifier.damage_factor = b.damage_factor
	...
	sp1.ts = store.tick_ts
	sp1.runs = 0
	sp2.ts = store.tick_ts
	sp2.runs = 0
...
function scripts.fireball_orc_shaman_meteorite.update(this, store)
	local b = this.bullet
	local mspeed = 10 * FPS
	...
	local damage_value = math.ceil(b.damage_factor * math.random(b.damage_min + b.damage_min_inc * b.level, b.damage_max + b.damage_max_inc * b.level))
	local mods = b.mods or (b.mod and {b.mod} or {})
	...
	if enemies then
		for i = 1, #enemies do
			...
			local unit_hit_fx = E:create_entity(b.unit_hit_fx)

			if enemy.unit.hit_offset then
				unit_hit_fx.pos:set(enemy.pos.x + enemy.unit.hit_offset.x, enemy.pos.y + enemy.unit.hit_offset.y)
			else
			...
			for j = 1, #mods do
				...
				if band(enemy.vis.bans, m.modifier.vis_flags) == 0 and band(enemy.vis.flags, m.modifier.vis_bans) == 0 then
					m.modifier.target_id = enemy.id
					m.modifier.source_id = this.id
					m.modifier.level = b.level
					m.modifier.damage_factor = b.damage_factor
...
function scripts.tower_bone_flingers.update(this, store)
	...
	local function spawn_filter_fn(e)
		return U.has_valid_rally_node_nearby(e.pos)
	end
	...
	while true do
		if not this.tower.blocked then
			...
			if ready_to_attack(aa, store, this.tower.cooldown_factor) then
				-- 为了尽量保证动画朝向一致性，前后的索敌逻辑需要统一。
				local seek_method_rand = math.random()
				local enemy

				if seek_method_rand < 0.5 then
					enemy = U.find_first_enemy_in_range_filter_off(tpos, a.range, aa.vis_flags, aa.vis_bans)
				else
					enemy = U.find_foremost_enemy_with_flying_preference_in_range_filter_off(tpos, a.range, aa.vis_flags, aa.vis_bans)
				end
				...
				if enemy then
					...
					if enemy.health.dead then
						local available_enemy

						if seek_method_rand < 0.5 then
							available_enemy = U.find_random_enemy(store.entities, tpos, 0, a.range, aa.vis_flags, aa.vis_bans)
						else
							available_enemy = U.find_foremost_enemy_with_flying_preference_in_range_filter_off(tpos, a.range, aa.vis_flags, aa.vis_bans)
						end

						if available_enemy then
							enemy = available_enemy
						end
					end
			...
			if not this.tower.blocked then
				for i = 1, attack_count do
					...
					if not aa.disabled and ready_to_attack(aa, store, tw.cooldown_factor) then
						...
						U.animation_start(this, ani_idle, nil, store.tick_ts, true, sid)
		...
		while true do
			if not this.tower.blocked then
				if not a_multi.disabled and ready_to_attack(a_multi, store, this.tower.cooldown_factor) then
					...
					U.animation_start(this, an, af, store.tick_ts, true, sid_m)
				...
				if ready_to_attack(a_normal, store, this.tower.cooldown_factor) then
					...
					U.y_wait_unconditional(store, a_normal.shoot_time)
...
scripts.soldier_ogre_shipwreck_red_goblin = {}

function scripts.soldier_ogre_shipwreck_red_goblin.insert(this, store)
	scripts.soldier_reinforcement.insert(this, store)
	this.nav_rally.pos:copy(this.pos)
	this.nav_rally.center:copy(this.pos)
		...
		if not this.tower.blocked then
			...
			if U.tower_ready_to_use_power(pow_n, an, store, this.tower) then
				...
				if not enemy then
					...
					U.y_wait_unconditional(store, an.shoot_time)
	...
	if source then
		if source.powers.engine.level > 0 then
			...
			if this.template_name == "missile_rr_nitro" then
				count = count + source.powers.nitro.fragment_count_inc
			end
...
function scripts.tower_spirit_mausoleum.update(this, store)
	local spritesGroup = "layers"
	local a = this.attacks
	local a1 = this.attacks.list[1]
	local a2 = this.attacks.list[2]
	local barrack = this.barrack
	local pow_g = this.powers.gargoyles
	local pow_p = this.powers.possession
	local pow_s = this.powers.spectral_communion
	...
	while true do
		if this.tower.blocked then
			...
			if pow_s.changed then
				...
				if not this._aura_spectral_communion then
					local aura = E:create_entity("aura_spectral_communion")
			...
			if pow_g.changed then
				...
				for i = 1, level do
					local sprite_idx = 4 + i

					if not this.render.sprites[sprite_idx].hidden then
						...
						gargoyle.nav_rally.pos, gargoyle.nav_rally.center = U.rally_formation_position(i, barrack, barrack.max_soldiers, barrack.rally_angle_offset)

						gargoyle.pos:copy(fx.pos)

						gargoyle.nav_rally.new = true
						barrack.soldiers[i] = gargoyle

						signal.emit("tower-spawn", this, gargoyle)

						gargoyles[#gargoyles + 1] = {sprite_idx, gargoyle}
					end
				end
			...
			for i = 1, barrack.max_soldiers do
				...
				if not s or s.health.dead and store.tick_ts - s.health.death_ts > s.health.dead_lifetime then
					...
					new_s.soldier.tower_soldier_idx = i
					new_s.nav_rally.pos, new_s.nav_rally.center = U.rally_formation_position(i, barrack, barrack.max_soldiers, barrack.rally_angle_offset)

					if s then
						new_s.pos = s.pos
						new_s.nav_rally.new = true
					else
						new_s.pos:copy(new_s.nav_rally.pos)
			...
			if ready_to_attack(a1, store, this.tower.cooldown_factor) then
				local target = U.find_first_enemy_in_range_filter_off(tpos(this), a.range, a1.vis_flags, a1.vis_bans)

				if not target and (#a1.stored_bullets >= a1.max_charges) then
					a1.ts = a1.ts + 0.1
				else
					local start_ts = store.tick_ts
					...
					if #a1.stored_bullets < a1.max_charges then
						S:queue(a1.sound)
						U.y_animation_play_group(this, a1.charge_animation, nil, store.tick_ts, 1, spritesGroup)
					...
					if target then
						...
						local target_expected_hp = target.health.hp
						local j = 1
...
function scripts.aura_spectral_communion.update(this, store)
	...
	local function filter_fn(v)
		return v.health.dead and band(v.health.last_damage_types, DAMAGE_EAT) == 0 and store.tick_ts - v.health.death_ts >= v.health.dead_lifetime - this.aura.cycle_time and not v._aura_spectral_communion_transferred
	end
	...
	while true do
		...
		if store.tick_ts - last_ts >= this.aura.cycle_time then
			...
			if #dead_enemies > 0 then
				local a = source.attacks.list[1]
				local start_offset = a.bullet_start_offset

				for i, dead in ipairs(dead_enemies) do
					dead._aura_spectral_communion_transferred = true

					local b = E:create_entity("bolt_tower_spirit_mausoleum")

					if dead.unit then
						b.pos:set(dead.pos.x + dead.unit.hit_offset.x, dead.pos.y + dead.unit.hit_offset.y)
					else
						b.pos:copy(dead.pos)
					end
...
function scripts.tower_spirit_mausoleum_bolt.update(this, store)
	local b = this.bullet
	local fm = this.force_motion
	local target
	...
	while true do
		...
		elseif store.tick_ts - b.ts < this.initial_impulse_duration then
			local t = store.tick_ts - b.ts
...
function scripts.mod_possession.insert(this, store)
	local target = store.entities[this.modifier.target_id]
	...
	if not SU.is_valid_betray_target(target) then
		return false
	end
	...
	this.betray.duration = this.betray_duration[this.modifier.level]
...
function scripts.mod_possession.update(this, store)
	...
	this.betray.ts = store.tick_ts

	SU.betray_enemy(target, this.betray)
...
function scripts.tower_goblirang.update(this, store)
	...
	local function shot_bullet(attack, shooter_idx, enemy, level, is_big)
		...
		epos.y = epos.y + enemy.unit.hit_offset.y

		local dir_x, dir_y = V.normalize(epos.x - start_x, (epos.y - start_y) / ellipse)

		rate = 1 - 0.25 * dir_y

		b.bullet.to:set(start_x + a.range * dir_x * rate, start_y + a.range * dir_y * rate * ellipse)
...
function scripts.goblirang.update(this, store)
	...
	while store.tick_ts - start_ts + store.tick_length <= flight_time do
		...
		local t = (store.tick_ts - start_ts) / flight_time
		...
		t = 1 - t * t
		this.pos.x = (b.to.x - b.from.x) * t + b.from.x
		this.pos.y = (b.to.y - b.from.y) * t + b.from.y

		if store.tick_ts - b.ts > b.damage_every then
			b.ts = store.tick_ts

			local targets = U.find_enemies_around_line(last_x, last_y, this.pos.x, this.pos.y, b.damage_radius, b.vis_flags, b.vis_bans)

			if targets then
				for i = 1, #targets do
					local t = targets[i]

					if not hit_enemies[t.id] then
						do
							local sfx = E:create_entity(b.hit_fx)
						...
						if mods then
							for j = 1, #mods do
								local mod_name = mods[j]
								local mt = E:get_template(mod_name)
								local mod_info = mt.modifier
								local chance = mod_info.chance or 1

								if U.flags_pass(t.vis, mod_info) and math.random() < chance then
									local mod = E:create_entity(mod_name)
...
function scripts.tower_ignis_altar.get_info(this)
	...
	local a = E:get_template("aura_bullet_ignis_altar")
	local o = scripts.tower_common.get_info(this)
	local count = math.floor(a.aura.duration / a.aura.cycle_time + 1e-8)

	o.damage_min = mod.damage_min * this.tower.damage_factor * count
	o.damage_max = mod.damage_max * this.tower.damage_factor * count
			...
			if not this.tower.blocked then
				if ready_to_attack(a1, store, this.tower.cooldown_factor) then
					...
					if target then
						...
						bullet.bullet.to = U.find_best_center_node_containing_point(pred_pos, E:get_template("aura_bullet_ignis_altar").aura.radius, target.nav_path.pi)
						bullet.bullet.source_id = this.id
						bullet.bullet.damage_factor = this.tower.damage_factor
...
function scripts.tower_shaolin.update(this, store)
	...
	local function spawn_pixies()
		for i = #this.pixies + 1, pow_t.base_count + pow_t.level do
...
scripts.mod_gold_indicator = {
	remove = function(this, store)
		...
		if target and target.health.dead and target.enemy.gold > 0 then
...
scripts.tower_blazing_watcher = {
	get_info = function(this)
		...
		o.damage_min = o.damage_min * this.attack_stage
		o.damage_max = o.damage_max * this.attack_stage
		o.cooldown = E:get_template("mod_tower_blazing_watcher_damage").dps.damage_every * this.tower.cooldown_factor
			...
			if this.tower.blocked then
				...
				if ready_to_attack(ab, store, this.tower.cooldown_factor) then
					if U.find_first_enemy_in_range_filter_off(tpos, a.range, ab.vis_flags, ab.vis_bans) then
						...
						U.y_wait_unconditional(store, fts(10) * this.tower.cooldown_factor)
						...
						if pow_e.level > 0 then
							...
							local _, targets = U.find_foremost_enemy_in_range_filter_off(tpos, a.range, nil, ab.vis_flags, ab.vis_bans)
...
scripts.bullet_tower_blazing_watcher = {
	update = function(this, store)
		...
		while store.tick_ts - s.ts < b.hit_time * tower.tower.cooldown_factor do
			coroutine.yield()
			update_sprite()
		end
		...
		while target and not target.health.dead and not this.force_stop_ray and source do
			...
			if attack_stage ~= tower.attack_stage then
				...
				if attack_stage >= 4 then
					...
					mod_dps.render.sprites[1].name = "blazing_watcher_hit_level4Run"
					this.render.sprites[2].name = "level4Run"
				elseif attack_stage == 3 then
					U.animation_start(this, "loop3", nil, store.tick_ts, true, 1)
				elseif attack_stage == 2 then
...
scripts.mod_tower_blazing_watcher_damage = {
	update = function(this, store)
		...
		this.pos = target.pos
		this.render.sprites[1].offset = target.unit.hit_offset
...
scripts.blazing_watcher_bolt_blast = {
	update = function(this, store)
		local b = this.bullet
		local dradius = b.damage_radius
		local dmin = b.damage_min * b.level * this.attack_stage
		local dmax = b.damage_max * b.level * this.attack_stage
...
function scripts.tower_swamp_monster.get_info(this)
	if this.tower_upgrade_persistent_data.current_mode == 0 then
		return scripts.tower_common.get_info(this)
	else
			...
			if current_mode == 0 then
				...
				if ready_to_attack(ab, store, this.tower.cooldown_factor) then
					...
					if target then
						...
						local start_offset = ab.bullet_start_offset
						local use_pow_i = pow_i.level > 0 and random() < pow_i.chance[pow_i.level]
						...
						local an, af = U.animation_name_facing_point(this, "shoot", target.pos, 3)

						U.animation_start_group(this, an, af, store.tick_ts, false, "layers")
						U.y_wait_unconditional(store, ab.shoot_time)

						-- retarget to find the available one
						do
							local new_target = U.detect_foremost_enemy_with_flying_preference_in_range_filter_off(tpos, this.attacks.range, ab.vis_flags, ab.vis_bans)
						...
						if use_pow_i then
							if band(target.vis.flags, F_FLYING) ~= 0 then
								...
								local pred_pos = U.calculate_enemy_ffe_pos(target, bl.bullet.flight_time)

								bl.bullet.to:copy(pred_pos)

								bl.bullet.level = pow_i.level
							end
						else
						...
						if pow_s.level > 0 and random() < pow_s.mod_chance_archer[pow_s.level] then
...
function scripts.tower_melting_furnace.update(this, store)
	local a = this.attacks
	local a_smash = a.list[1]
	local a_coal = a.list[2]
	local a_buff = a.list[3]
	local a_fuel = a.list[4]
		...
		if not this.tower.blocked then
			...
			if U.tower_ready_to_use_power(pow_heat, a_buff, store, this.tower) then
				...
				local towers = table.filter(store.towers, function(_, e)
					...
					return e.tower and e.tower.can_be_mod and U.is_inside_ellipse(tpos, e.pos, a_buff.range) and (#mods == 0 or mods[1].modifier.level < pow_heat.level)
				end)

				for _, tower in ipairs(towers) do
					local new_mod = E:create_entity(a_buff.mod)

					new_mod.modifier.level = pow_heat.level
					new_mod.damage_factor = pow_heat.level * (new_mod.damage_factor - 1) + 1
					new_mod.modifier.target_id = tower.id
					new_mod.modifier.source_id = this.id
					new_mod.pos = tower.pos
			...
			if U.tower_ready_to_use_power(pow_coal, a_coal, store, this.tower) then
				...
				if trigger_enemy then
					...
					U.animation_start_default(this, "shootFissure", nil, store.tick_ts)

					this.render.sprites[10].hidden = false
					this.tween.disabled = false
					this.tween.props[1].ts = store.tick_ts

					U.y_wait_unconditional(store, a_coal.hit_time)
					S:queue(a_coal.sound)

					local new_enemy = U.detect_foremost_enemy_in_range_filter_off(tpos, a.range, a_coal.vis_flags, a_coal.vis_bans)

					if new_enemy then
						trigger_enemy = new_enemy
					end

					local fragment_count = pow_coal.fragment_count_base + pow_coal.fragment_count_inc * pow_coal.level
					local dest = U.calculate_enemy_ffe_pos(trigger_enemy, E:get_template(a_coal.bullet).bullet.flight_time + fts(fragment_count))
					local nearest_nodes = P:nearest_nodes(dest.x, dest.y, {trigger_enemy.nav_path.pi})
					local n_offset = math.ceil(fragment_count / 2)
					local ni_offset = n_offset * a_coal.fragment_node_spread
					local ni, _, pi

					if #nearest_nodes > 0 then
						pi, _, ni = unpack(nearest_nodes[1])
						ni_offset = km.clamp(fragment_count * a_coal.fragment_node_spread + 1 - ni, a_coal.fragment_node_spread + #P.paths[pi][1] - ni, ni_offset)
					end
					...
					for i = 1, fragment_count do
						...
						if #nearest_nodes > 0 then
							bf_dest = P:node_pos(pi, 1, ni + ni_offset - i * a_coal.fragment_node_spread)
						else
							bf_dest = U.point_on_ellipse(dest, (50 * math.random() + 45) / 2, 2 * math.pi * i / fragment_count)
						end

						bf_dest.x = bf_dest.x + U.frandom(-a_coal.fragment_pos_spread.x, a_coal.fragment_pos_spread.x)
						bf_dest.y = bf_dest.y + U.frandom(-a_coal.fragment_pos_spread.y, a_coal.fragment_pos_spread.y)

						local b = E:create_entity(a_coal.bullet)

						b.pos.x = this.pos.x + (n_offset - i) * a_coal.bullet_start_offset.x
						b.pos.y = this.pos.y + a_coal.bullet_start_offset.y + math.random(-5, 5)
						b.bullet.damage_factor = this.tower.damage_factor

						b.bullet.from:copy(b.pos)
						b.bullet.to:copy(bf_dest)
			...
			if not a_fuel.boost and U.tower_ready_to_use_power(pow_fuel, a_fuel, store, this.tower) then
				...
				if trigger and U.is_inside_ellipse(tpos, U.calculate_enemy_ffe_pos(trigger, fts(45)), a.range) then
					...
					U.animation_start_default(this, "bfIntro", nil, store.tick_ts)
					U.y_animation_wait(this, 2)
					S:queue("MeltingFurnaceBurningFuel")

					local towers = table.filter(store.towers, function(_, e)
						return e.tower.can_be_mod and U.is_inside_ellipse(tpos, e.pos, a_buff.range)
					end)

					for i = 1, #towers do
						local tower = towers[i]
						local mod = E:create_entity(a_fuel.mod)
					...
					U.y_wait_unconditional(store, fts(5) * this.tower.cooldown_factor)
				else
					a_fuel.ts = a_fuel.ts + 0.1
				end
			end

			if ready_to_attack(a_smash, store, this.tower.cooldown_factor) then
				if U.find_first_enemy_in_range_filter_off(tpos, a.range, a_smash.damage_flags, a_smash.damage_bans) then
					...
					if enemies then
						...
						for i = 1, #enemies do
							local enemy = enemies[i]
							local value = (UP:get_upgrade("engineer_efficiency") and a_smash.damage_max or math.random(a_smash.damage_min, a_smash.damage_max)) * this.tower.damage_factor
							local d = E.assign_damage(a_smash.damage_type, value, this.id, enemy.id)

							d.reduce_armor = a_smash.reduce_armor
							...
							for j = 1, #a_smash.mods do
								local m = E:create_entity(a_smash.mods[j])
					...
					local fx_points = U.get_path_fx_points(this, 55)

					for _, p in ipairs(fx_points) do
						if band(p.terrain, TERRAIN_WATER) ~= 0 then
							local smoke = E:create_entity("decal_dwaarp_smoke_water")
					...
					U.animation_start_default(this, a_fuel.boost and "bfLoop" or "idle", nil, store.tick_ts, true)
				else
					a_smash.ts = a_smash.ts + 0.1
				end
			end
		end

		if not a_fuel.boost and this.render.sprites[2].name == "bfLoop" then
			U.animation_start_default(this, "idle", nil, store.tick_ts, true)
		end
...
function scripts.lava_furnace.update(this, store)
	local a = this.aura
	local damage_every = a.cycle_time
	local damage_factor = a.damage_factor
	local dps_ts = a.ts

	a.damage_min = a.damage_min + a.level * a.damage_inc
	a.damage_max = a.damage_max + a.level * a.damage_inc

	while store.tick_ts - a.ts <= this.actual_duration do
		...
		if store.tick_ts - dps_ts > damage_every then
			...
			local enemies = U.find_enemies_in_range_filter_off(this.pos, a.radius, a.vis_flags, a.vis_bans)

			if enemies then
				for _, enemy in ipairs(enemies) do
					local d_value = math.random(a.damage_min, a.damage_max) * damage_factor
					local d = E.assign_damage(a.damage_type, d_value, this.id, enemy.id)

					queue_damage(store, d)

					d = E.assign_damage(DAMAGE_ARMOR, this.damage_armor, this.id, enemy.id)
...
function scripts.tower_wicked_sisters.get_info(this)
	local witch = E:get_template("soldier_wicked_sisters")

	if this.tower_upgrade_persistent_data.current_mode == 0 then
		local mod = E:get_template("mod_wicked_sister_poison")
		local count = math.floor(mod.modifier.duration / mod.dps.damage_every + 1e-8)
		local damage_min = mod.dps.damage_min * count * this.tower.damage_factor
		local damage_max = mod.dps.damage_max * count * this.tower.damage_factor

		return {
			type = STATS_TYPE_TOWER_MAGE,
			damage_min = damage_min,
			damage_max = damage_max,
			-- 我们暂时没有毒伤的图标，这里使用真伤作为代替
			damage_type = DAMAGE_TRUE,
			range = this.barrack.rally_range,
			cooldown = witch.attacks.list[1].cooldown * this.tower.cooldown_factor
		}
	else
		local b = E:get_template("wicked_sisters_proy_pink")

		return {
			type = STATS_TYPE_TOWER_MAGE,
			damage_min = b.bullet.damage_min * this.tower.damage_factor,
			damage_max = b.bullet.damage_max * this.tower.damage_factor,
			damage_type = b.bullet.damage_type,
			range = this.barrack.rally_range,
			cooldown = witch.attacks.list[1].cooldown * this.tower.cooldown_factor
		}
...
function scripts.tower_wicked_sisters.update(this, store)
	...
	if not witch then
		...
		witch.pos:set(this.pos.x, this.pos.y + 16)
	...
	local pow_s = this.powers.silent
	local sa = this.attacks.list[1]
	local pow_f = this.powers.frog
	local pow_r = this.powers.range
	local tpos = tpos(this)
	local stir_ts = store.tick_ts - this.stir_cooldown
	local kid_ts = store.tick_ts - this.kid_cooldown
	...
	while true do
		if this.change_mode then
			...
			if current_mode == 0 then
				...
				U.animation_start_specific(this, "Violet", nil, store.tick_ts, true, 4)
				...
				U.y_animation_play_once_specific_no_flip(this, "toGreen", store.tick_ts, 4)
				U.animation_start_specific(this, "Green", nil, store.tick_ts, true, 4)
		...
		if pow_s.changed then
			pow_s.changed = nil
			witch.powers.silent.level = pow_s.level
			witch.powers.silent.changed = true
		end

		if pow_f.changed then
			pow_f.changed = nil
			witch.powers.frog.level = pow_f.level
			witch.powers.frog.changed = true
		end

		if pow_r.changed then
			pow_r.changed = nil

			SU.insert_tower_range_buff(this, pow_r.range_factor[pow_r.level] / pow_r.last_range_factor, true)

			pow_r.last_range_factor = pow_r.range_factor[pow_r.level]
		end
		...
		if not this.tower.blocked then
			if U.tower_ready_to_use_power(pow_s, sa, store, this.tower) then
				...
				if enemy then
					...
					local totem_pos = P:node_pos_ref(enemy.nav_path.pi, enemy.nav_path.spi, enemy.nav_path.ni)
					local b2 = E:create_entity(sa.bullet)

					b2.pos:copy(totem_pos)

					b2.aura.level = pow_s.level
					b2.aura.source_id = this.id
					b2.aura.damage_factor = this.tower.damage_factor
		...
		if store.tick_ts - stir_ts > this.stir_cooldown then
			U.animation_start_once_specific_no_flip(this, "stir", store.tick_ts, 5)

			stir_ts = store.tick_ts
		end

		if this.render.sprites[5].name ~= "idle" and U.animation_finished(this, 5) then
			U.animation_start_specific(this, "idle", nil, store.tick_ts, true, 5)
		end

		if store.tick_ts - kid_ts > this.kid_cooldown then
			U.animation_start_once_specific_no_flip(this, "run", store.tick_ts, 8)

			kid_ts = store.tick_ts
		end

		if this.render.sprites[8].name ~= "idle" and U.animation_finished(this, 8) then
			U.animation_start_specific(this, "idle", nil, store.tick_ts, true, 8)
		end
...
function scripts.soldier_wicked_sisters.update(this, store)
	...
	local function score_fn(e)
		return -e.id
	end

	local function find_target(a)
		local candidates = U.find_enemies_in_range_filter_off(this.pos, a.max_range, a.vis_flags, a.vis_bans)

		if not candidates then
			return nil
		end

		return table.find_best(candidates, score_fn)
	end
	...
	while true do
		...
		while r.new do
			...
			U.animation_start_specific(this, an, af, store.tick_ts, true, 1)
		...
		if U.tower_ready_to_use_power(pow_m, am, store, tw) then
			...
			if target then
				...
				local an, af = U.animation_name_facing_point_simple(this, am.animation, target.pos)

				U.animation_start_once_specific(this, an, af, store.tick_ts, 1)
				U.y_wait_unconditional(store, am.hit_time)

				local new_target = find_target(am)
		...
		if ready_to_attack(ab, store, tw.cooldown_factor) then
			local trigger = find_target(ab)

			if trigger then
				ab.ts = store.tick_ts

				local wick_mode = this.wick_mode
				local wick_idx = wick_mode + 1
				local an, af = U.animation_name_facing_point_simple(this, ab.animations[wick_idx], trigger.pos)

				U.animation_start_once_specific(this, an, af, store.tick_ts, 1)
				U.y_wait_unconditional(store, ab.hit_time)

				local new_trigger = find_target(ab)

				if new_trigger then
					trigger = new_trigger
				end

				local b = E:create_entity(ab.bullet[wick_idx])
				...
				if wick_mode == 1 and random() < 0.6 and band(trigger.vis.flags, E:get_template("mod_wicked_sisters_stun").modifier.vis_bans) == 0 and band(trigger.vis.bans, E:get_template("mod_wicked_sisters_stun").modifier.vis_flags) == 0 then
		...
		U.animation_start_specific(this, "idle", nil, store.tick_ts, true, 1)
		coroutine.yield()
	end
end
...
function scripts.aura_totem_wicked_sisters.update(this, store)
	...
	this.aura.ts = store.tick_ts
	a.duration = a.duration + a.duration_inc * a.level

	local last_ts = store.tick_ts - a.cycle_time
...
function scripts.tower_sandworm.get_info(this)
	local a = E:get_template(this.attacks.list[1].bullet)
	local count = math.floor(a.aura.duration / a.aura.cycle_time + 1e-8)
	local damage_min = a.aura.damage_min * this.tower.damage_factor * count
	local damage_max = a.aura.damage_max * this.tower.damage_factor * count

	return {
		type = STATS_TYPE_TOWER,
		damage_min = damage_min,
		damage_max = damage_max,
		damage_type = a.aura.damage_type,
		range = this.attacks.range,
		cooldown = this.attacks.list[1].cooldown * this.tower.cooldown_factor
	}
end
	...
	while true do
		if not this.tower.blocked then
			...
			if pow_w.level > 0 and pow_w.changed then
				pow_w.changed = nil
				aw.cooldown = pow_w.cooldown[pow_w.level]
				aw.entity = "soldier_tower_sandworm_" .. pow_w.level
			end
			...
			if U.tower_ready_to_use_power(pow_e, ae, store, this.tower) then
				...
				if enemies then
					for i = 1, #enemies do
						if enemies[i].health.hp > BIG_ENEMY_HP then
							satisfied = true
				...
				if satisfied then
					...
					U.animation_start_once_specific_no_flip(this, "instakill", store.tick_ts, 2)
					S:queue("sandwormEatIn")
					U.y_wait_unconditional(store, ab.shoot_time)

					local new_enemy = U.find_foremost_enemy_with_max_coverage_in_range_filter_on(tpos, a.range, nil, ae.vis_flags, ae.vis_bans, E:get_template(ae.bullet).radius, function(e)
						return e.health.hp > BIG_ENEMY_HP
					end)
			...
			-- 普攻
			if ready_to_attack(ab, store, this.tower.cooldown_factor) then
				local targets = U.find_enemies_in_range_filter_off(tpos, a.range, ab.vis_flags, ab.vis_bans)
				...
				if targets then
					...
					U.animation_start_once_specific_no_flip(this, "shoot", store.tick_ts, 2)
					U.y_wait_unconditional(store, ab.shoot_time)
					S:queue(ab.sound)

					targets = U.find_enemies_in_range_filter_off(tpos, a.range, ab.vis_flags, ab.vis_bans)

					if targets then
						target = table.find_best(targets, score_fn)
					end

					local a = E:create_entity(ab.bullet)

					a.aura.damage_factor = this.tower.damage_factor
			...
			if U.tower_ready_to_use_power(pow_w, aw, store, this.tower) then
				...
				local e = E:create_entity(aw.entity)

				if target then
					e.pos = U.calculate_enemy_ffe_pos(target, fts(11))
					...
					for i = 1, #nodes do
						...
						if tpos:dist(node_pos) <= a.range and U.has_valid_rally_node_nearby(node_pos) then
							e.pos:copy(node_pos)

							e.nav_path.pi = nodes[i][1]
							e.nav_path.spi = nodes[i][2]
							e.nav_path.ni = nodes[i][3]
			...
			if U.tower_ready_to_use_power(pow_s, as, store, this.tower) then
				...
				if target then
					...
					U.animation_start_once_specific_no_flip(this, "spit", store.tick_ts, 2)
					U.y_wait_unconditional(store, as.shoot_time)

					local new_target = U.detect_foremost_enemy_in_range_filter_off(tpos, a.range, as.vis_flags, as.vis_bans, filter_fn)
...
-- 沙虫：普通攻击效果
scripts.aura_tower_sandworm = {
	update = function(this, store)
		local a = this.aura

		U.y_animation_play_once_specific_no_flip(this, "in", store.tick_ts, 1)

		a.ts = store.tick_ts

		local last_ts = store.tick_ts - a.cycle_time

		U.animation_start_specific(this, "run", nil, store.tick_ts, true, 1)
		...
		while store.tick_ts - a.ts < a.duration do
			if store.tick_ts - last_ts >= a.cycle_time then
				...
				if enemies then
					for i = 1, #enemies do
						local d = E.assign_damage(a.damage_type, math.random(a.damage_min, a.damage_max) * a.damage_factor, a.source_id, enemies[i].id)
...
scripts.controller_tower_sandworm_eat = {
	update = function(this, store)
		-- 先进行沙土预警动画
		U.y_animation_play_once_specific_no_flip(this, "worm_nest_level4_instakill_dust_run", store.tick_ts, 1)
		...
		if enemies then
			for i = 1, #enemies do
				local d = E.assign_damage(this.damage_type, 1, this.source_id, enemies[i].id)
		...
		if soldiers then
			for i = 1, #soldiers do
				local d = E.assign_damage(this.damage_type, 1, this.source_id, soldiers[i].id)
		...
		decal.render.sprites[1].ts = store.tick_ts
		decal.render.sprites[1].fps = this.render.sprites[1].fps
...
scripts.aura_bomb_tower_sandworm_spit = {
	update = function(this, store)
		local fade_in_ended = false
		local a = this.aura
		local last_ts = store.tick_ts - a.cycle_time

		a.ts = store.tick_ts

		U.animation_start_once_specific_no_flip(this, "in", store.tick_ts, 1)

		while store.tick_ts - a.ts < this.actual_duration do
			if not fade_in_ended then
				...
				if enemies then
					for i = 1, #enemies do
						...
						for j = 1, #a.mods do
							...
							if band(enemy.vis.flags, E:get_template(mod).modifier.vis_bans) == 0 and band(enemy.vis.bans, E:get_template(mod).modifier.vis_flags) == 0 then
