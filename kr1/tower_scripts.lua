...
scripts.tower_musketeer = {
    update = function(this, store)
        ...
        local function shot_animation(attack, shooter_idx, enemy, animation)
            local ssid = shooter_sids[shooter_idx]
            local an, af, ai = animation_name_facing_point(this, animation or attack.animation, enemy.pos, ssid,
                sprites[ssid].offset)
            ...
            if tw.blocked then
                ...
                if pow_sn.level > 0 then
                    for _, ax in pairs({asi, asn}) do
                        if (ax.chance == 1 or random() < ax.chance) and
                ...
                if ready_to_use_power(pow_sh, ash, store, tw.cooldown_factor) then
                    ...
                    if not enemy then
                        ...
                        local src_pos = v(this.pos.x + soffset.x + boffset.x * (shooting_right and 1 or -1),
            ...
            if tw.blocked then
                ...
                if ready_to_use_power(pow_m, ma, store, tw.cooldown_factor) then
                    ...
                    if not enemy then
                        ...
                        local an, af = animation_name_facing_point(this, "multishot_start", enemy.pos, shooter_sid,
                            start_offset)
                        ...
                        an, af = animation_name_facing_point(this, "multishot_loop", enemy.pos, shooter_sid,
                            start_offset)
                        ...
                        for i = 1, ma.shots + pow_m.level * ma.shots_inc do
                            ...
                            if last_enemy.health.dead then
                                enemy = U.detect_foremost_enemy_with_flying_preference_in_range_filter_off(origin,
                                    range, ma.vis_flags, ma.vis_bans)
                            end
                            ...
                            if enemy then
                                last_enemy = enemy
                                enemy_id = enemy.id
                                shoot_pos = v(enemy.pos.x + enemy.unit.hit_offset.x,
                        ...
                        local an, af = animation_name_facing_point(this, "multishot_end", last_enemy.pos, shooter_sid,
                            start_offset)
            ...
            if tw.blocked then
                ...
                if ready_to_attack(aa, store, tw.cooldown_factor) then
                    ...
                    if not enemy then
                        ...
                        local shooter_sid = shooter_sprite_ids[shooter_idx]
                        local start_offset = aa.bullet_start_offset[shooter_idx]
                        local an, af = animation_name_facing_point(this, aa.animation, enemy.pos, shooter_sid,
                            start_offset)
            ...
            if this.tower.blocked then
                ...
                if ready_to_attack(a, store, tw.cooldown_factor) then
                    ...
                    if not enemy then
                        ...
                        local an, af = animation_name_facing_point(this, a.animation, enemy.pos, shooter_sid,
                            start_offset)
            ...
            if tw.blocked then
                ...
                if ready_to_attack(ba, store, tw.cooldown_factor) then
                    ...
                    if enemy then
                        ...
                        local an, af, ai = animation_name_facing_point(this, ba.animations[aidx], enemy.pos,
                            shooter_sid, fo)
                ...
                for i, aa in pairs(attacks) do
                    ...
                    if (pow and ready_to_use_power(pow, aa, store, this.tower.cooldown_factor)) or
                        ...
                        if not enemy then
                            ...
                            local soffset = this.render.sprites[shooter_sid].offset
                            local an, af, ai = animation_name_facing_point(this, aa.animation, enemy.pos, shooter_sid,
                                soffset)
        ...
        while true do
            if this.tower.blocked then
                ...
                if ready_to_use_power(pow_t, ta, store, this.tower.cooldown_factor) then
                    local target = U.find_foremost_enemy_in_range_filter_on(tpos(this), a.range, false, ta.vis_flags,
        ...
        while true do
            if this.tower.blocked then
                ...
                if ready_to_use_power(pow_p, pa, store, this.tower.cooldown_factor) then
                    local enemy = U.find_foremost_enemy_in_range_filter_off(tpos(this), a.range, false, pa.vis_flags,
                        pa.vis_bans)
                ...
                if ready_to_attack(ba, store, this.tower.cooldown_factor) then
                    local enemy = U.detect_foremost_enemy_in_range_filter_off(tpos(this), a.range, ba.vis_flags,
                        ba.vis_bans)
            ...
            if this.tower.blocked then
                ...
                if ready_to_use_power(pow_m, am, store, this.tower.cooldown_factor) then
                    ...
                    if not trigger then
                        ...
                        local enemy = U.detect_foremost_enemy_in_range_filter_off(tpos(this), am.range, am.vis_flags,
                            am.vis_bans)
                ...
                if ready_to_use_power(pow_c, aa, store, this.tower.cooldown_factor) then
                    ...
                    if not trigger then
                        ...
                        local enemy = U.detect_foremost_enemy_in_range_filter_off(tpos(this), a.range, aa.vis_flags,
                            aa.vis_bans)
    ...
    update = function(this, store)
        ...
        while true do
            ...
            if store.tick_ts - last_hit_ts >= this.aura.cycle_time then
                ...
                local targets = U.find_enemies_in_range_filter_on(this.pos, this.aura.radius, this.aura.vis_flags,
            ...
            if this.tower.blocked then
                ...
                if not drill_ready and not lava_ready and not std_ready then
                    ...
                    if drill_ready then
                        ...
                        local trigger_enemy = U.detect_foremost_enemy_in_range_filter_on(tpos(this), a.range,
                        ...
                        if not trigger_enemy then
                            ...
                            if trigger_enemy and trigger_enemy.health.hp > 0 then
                                ...
                                enemy = U.detect_foremost_enemy_in_range_filter_off(tpos(this), a.range, da.vis_flags,
                                    da.vis_bans)
                            end
                    ...
                    local trigger_enemy = U.find_first_enemy_in_range_filter_off(tpos(this), trigger_range,
                        aa.vis_flags, aa.vis_bans)
                    ...
                    if trigger_enemy then
                        ...
                        local enemies = U.find_enemies_in_range_filter_off(tpos(this), a.range, aa.damage_flags,
                            aa.damage_bans)
    ...
    update = function(this, store)
        ...
        local function do_attack(at)
            ...
            if target then
                ...
                local nt, _, nt_pos = U.find_foremost_enemy_with_max_coverage_in_range_filter_on(tpos(this), a.range,
                    at.node_prediction, at.vis_flags, at.vis_bans, b.bullet.damage_radius, filter_faerie)
            ...
            if this.tower.blocked then
                ...
                if loaded == "clobber" then
                    ...
                    if U.has_enough_enemies_in_range(store, tpos(this), 0, ca.range, ca.vis_flags, ca.vis_bans, nil,
                        ...
                        queue_insert(store, fx)
                        local targets = U.find_enemies_in_range_filter_off(tpos(this), ca.damage_radius, ca.vis_flags,
                            ca.vis_bans)
    ...
    update = function(this, store)
        ...
        local function target_after_check_thor()
            ...
            if thor.health.hp == thor.health.hp_max then
                local bounce_target = U.find_enemies_in_range_filter_off(thor.pos,
        ...
        while true do
            if this.tower.blocked then
                ...
                if ready_to_attack(ar, store, this.tower.cooldown_factor) then
                    local target = U.detect_foremost_enemy_in_range_filter_off(tpos(this), ar.range, ar.vis_flags,
                        ar.vis_bans)
                    ...
                    if not target then
                        ...
                        if target.health.dead or not store.entities[target.id] or
                            ...
                            target = U.detect_foremost_enemy_in_range_filter_off(tpos(this), ar.range, ar.vis_flags,
                                ar.vis_bans)
                        end
    ...
    while true do
        ...
        if pow_m.level > 0 then
            ...
            if store.tick_ts - am.ts > am.cooldown * this.cooldown_factor then
                local target = U.detect_foremost_enemy_in_range_filter_off(this.pos, am.max_range, am.vis_flags,
                    am.vis_bans)
                ...
                if not target then
                    ...
                    for i = 1, fire_loops do
                        ...
                        for hi, ht in ipairs(am.hit_times) do
                            ...
                            b.bullet.from = vclone(b.pos)
                            b.bullet.to = v(b.pos.x + (af and -1 or 1) * am.launch_vector.x,
                            ...
                            target = U.detect_foremost_enemy_in_range_filter_off(this.pos, am.max_range, am.vis_flags,
                                am.vis_bans)
...
scripts.tower_frankenstein = {
    ...
    update = function(this, store)
        ...
        local function target_after_check_thor()
            ...
            if thor.health.hp == thor.health.hp_max then
                local bounce_target = U.find_first_enemy_in_range_filter_off(thor.pos, rb.bounce_range * 2,
        ...
        while true do
            if this.tower.blocked then
                ...
                if ready_to_attack(ra, store, this.tower.cooldown_factor) then
                    local enemy = U.detect_foremost_enemy_in_range_filter_off(tpos(this), at.range, ra.vis_flags,
                        ra.vis_bans)
                    if not enemy or enemy.health.dead then
                        ...
                        if not enemy then
                            ...
                            if frankie and not frankie.health.dead then
                                enemy = U.detect_foremost_enemy_in_range_filter_off(frankie.pos, rb.bounce_range,
                                    ra.vis_flags, ra.vis_bans)
                    ...
                    if not enemy then
                        ...
                        if not enemy or store.entities[enemy.id] == nil or enemy.health.dead or
                            ...
                            enemy = U.detect_foremost_enemy_in_range_filter_off(tpos(this), at.range, ra.vis_flags,
                                ra.vis_bans)
                        end
...
function scripts.druid_shooter_sylvan.update(this, store)
    ...
    while true do
        ...
        elseif store.tick_ts - a.ts > a.cooldown * this.owner.tower.cooldown_factor then
            local target, enemies = U.find_foremost_enemy_in_range_filter_on(tpos(this.owner), a.range, nil,
...
function scripts.mod_druid_sylvan.update(this, store)
    ...
    if not target or not target.health or target.health.dead then
        if target then
            local new_target = U.find_first_enemy_in_range_filter_on(target.pos, a.max_range, a.vis_flags, a.vis_bans,
                function(v)
    ...
    while true do
        ...
        if store.tick_ts - ray_ts > this.ray_cooldown then
            ...
            if dhp > 0 then
                ...
                local targets = U.find_enemies_in_range_filter_on(target.pos, a.max_range, a.vis_flags, a.vis_bans,
                    function(v)
                        return not U.has_modifier(store, v, "mod_druid_sylvan")
                    end)
                ...
                if targets then
                    for _, t in pairs(targets) do
                        ...
                        b.bullet.source_id = this.id
                        b.bullet.from = v(target.pos.x + target.unit.mod_offset.x,
    ...
    while true do
        if this.tower.blocked then
            ...
            if ready_to_attack(ba, store, this.tower.cooldown_factor) then
                ...
                target = U.detect_foremost_enemy_in_range_filter_on(tpos(this), a.range, ba.vis_flags, ba.vis_bans,
                    filter_faerie)
                ...
                if target then
                    ...
                    target, _, pred_pos = U.find_foremost_enemy_with_max_coverage_in_range_filter_on(tpos(this),
        ...
        if this.tower.blocked then
            ...
            for i, aa in pairs(attacks) do
                ...
                if aa and (not pow or pow.level > 0) and aa.cooldown and
                    ...
                    local trigger, enemies, trigger_pos = U.find_foremost_enemy_in_range_filter_off(tpos(this),
                    ...
                    if not trigger then
                        ...
                        if aa == ab then
                            ...
                            local _, enemies, pred_pos = U.find_foremost_enemy_in_range_filter_off(tpos(this), aa.range,
                            ...
                            for bullet_idx, target_position_data in ipairs(target_positions) do
                                ...
                                y_wait(store, aa.time_between_bombs * tw.cooldown_factor)
                            end
                        ...
                        elseif aa == am then
                            ...
                            local enemy = U.detect_foremost_enemy_in_range_filter_off(tpos(this), aa.range,
                                aa.vis_flags, aa.vis_bans)
...
function scripts.tower_dark_elf.update(this, store)
    ...
    local function find_target_to_kill(node_prediction)
        local target_to_kill, targets = U.find_foremost_enemy_with_flying_preference_in_range_filter_off(tpos(this),
            this.attacks.range, attack.vis_flags, attack.vis_bans)
        local d = E:create_entity("damage")
    ...
    local function find_target(attack, node_prediction)
        ...
        elseif current_mode == MODE_FIND_MAXHP then
            local target = U.find_biggest_enemy_in_range_filter_off(tpos(this), this.attacks.range, attack.vis_flags,
                attack.vis_bans)
            if target then
    ...
    while true do
        if this.tower.blocked then
            ...
            if store.tick_ts - attack.ts > attack.cooldown * this.tower.cooldown_factor then
                ...
                if target then
                    bullet.bullet.to = v(target.pos.x + target.unit.hit_offset.x,
                        target.pos.y + target.unit.hit_offset.y)
            ...
            if store.tick_ts - last_ts > this.tower.long_idle_cooldown then
                local an, af = animation_name_facing_point(this, "idle", this.tower.long_idle_pos,
            ...
            if tower then
                ...
                if skill_buff and skill_buff.level > 0 and skill_buff.times < skill_buff.max_times then
                    if target.health.dead or U.predict_damage(target, d) >= target.health.hp then
                        ...
                        soul_mod.pos = v(target.pos.x + target.unit.hit_offset.x,
                            target.pos.y + target.unit.hit_offset.y)
            ...
            for i, aa in pairs(attacks) do
                ...
                if (not pow or pow.level > 0) and ready_to_attack(aa, store, this.tower.cooldown_factor) then
                    ...
                    if aa == ag then
                        ...
                        local enemy = U.detect_foremost_enemy_in_range_filter_on(tpos(this), a.range * 1.2,
...
function scripts.bullet_tower_necromancer.update(this, store)
    ...
    local function find_target()
        ...
        local target = U.detect_foremost_enemy_in_range_filter_off(tpos(this.source), this.source.attacks.range,
            attack.vis_flags, attack.vis_bans)
...
function scripts.aura_tower_necromancer_skill_debuff.update(this, store)
    ...
    while true do
        ...
        if store.tick_ts - last_hit_ts >= this.aura.cycle_time then
            ...
            local targets = U.find_enemies_in_range_filter_off(this.pos, this.aura.radius, this.aura.enemy_vis_flags,
                this.aura.enemy_vis_bans)
            if targets then
                for i, target in ipairs(targets) do
    ...
    while true do
        ...
        for _, panda in pairs(this.pandas) do
            ...
            if spr.name == "spawn_end" and animation_finished(this, panda.render) then
                animation_start(this, this.render.sprites[panda.render].angles.idle[1], nil, store.tick_ts, false,
                    panda.render, true)
        ...
        for i, panda in pairs(this.pandas) do
            ...
            if spr.name == "spell" and animation_finished(this, panda.render) then
                local an, af = animation_name_facing_point(this, this.render.sprites[panda.render].angles.idle[1],
        ...
        for _, panda in pairs(this.pandas) do
            ...
            if cfg and store.tick_ts >= cfg.shoot_ts then
                ...
                if enemy then
                    ...
                    local an, af = animation_name_facing_point(this, "shoot", enemy.pos, cfg.shooter_sid,
                        cfg.start_offset)
...
function scripts.bullet_tower_pandas_air.update(this, store)
    ...
    if this.bounces < this.max_bounces[b.level] then
        local targets = U.find_enemies_in_range_filter_on(this.pos, this.bounce_range, b.vis_flags, b.vis_bans,
            function(v)
...
function scripts.soldier_tower_pandas.insert(this, store)
    ...
    if this.info and this.info.random_name_format then
        this.info.i18n_key = string.format(string.gsub(this.info.random_name_format, "_NAME", ""),
            random(this.info.random_name_count))
    end
    ...
    while true do
        if this.tower.blocked then
                ...
                if not aa.disabled and ready_to_attack(aa, store, this.tower.cooldown_factor) and store.tick_ts -
                    ...
                    if aa == as then
                        ...
                        if not enemy then
                            ...
                            local an, af, ai = animation_name_facing_point(this, aa.animation_start, enemy.pos,
                            ...
                            local an, af, ai = animation_name_facing_point(this, aa.animation_loop, enemy.pos,
                                this.render.sid_mage, this.mage_offset)
                            ...
                            local an, af, ai = animation_name_facing_point(this, aa.animation_end, a._last_target_pos,
                                this.render.sid_mage, this.mage_offset)
                            ...
                            local soffset = this.shooter_offset
                            local an, af, ai = animation_name_facing_point(this, "idle", a._last_target_pos,
            ...
            if store.tick_ts - idle_ts > this.tower.long_idle_cooldown then
                local an, af, ai = animation_name_facing_point(this, "idle", this.tower.long_idle_pos,
    ...
    while store.tick_ts - start_ts < ray_duration and target and not this.force_stop_ray and source do
        if target.health.dead then
            if not this.chain_pos or this.chain_pos == 1 then
                ...
                local explosion_targets = U.find_enemies_between_range_filter_off(explosion_fx.pos,
                    this.explosion_radius, F_AREA, F_NONE)
            ...
            if not enemy then
                ...
                if enemies then
                    for i = 1, shots do
                        ...
                        if enemy.health and not enemy.health.dead then
                            bullet.bullet.to = v(enemy.pos.x + enemy.unit.hit_offset.x,
                                enemy.pos.y + enemy.unit.hit_offset.y)
                            bullet.bullet.target_id = enemy.id
                            if pow_s.level > 0 then
    ...
    while true do
        ...
        if this.tower.blocked then
            ...
            if bba.cooldown and ready_to_attack(bba, store, tw.cooldown_factor) then
                local _, enemies, pred_pos = U.find_foremost_enemy_in_range_filter_off(this.pos, bba.range,
            ...
            if at then
                local trigger_enemy = U.detect_foremost_enemy_with_flying_preference_in_range_filter_off(tpos(this),
                    a.range, at.vis_flags, at.vis_bans)
                if not trigger_enemy then
                    at.ts = at.ts + fts(10)
                else
                    at.ts = store.tick_ts
            ...
            while true do
                ...
                if ready_to_attack(aa, store, tw.cooldown_factor) then
                    ...
                    if not trigger_enemy then
                        ...
                        local an, af = animation_name_facing_point(this, aa.animation, trigger_enemy.pos, shooter_sid,
                            s.offset)
                        animation_start(this, an, af, store.tick_ts, 1, shooter_sid)
...
function scripts.tower_royal_archers_pow_rapacious_hunter_tamer.update(this, store)
    ...
    while true do
        ...
        if not this.entity_spawned then
            local enemy, enemies = U.find_foremost_enemy_in_range_filter_off(tpos(this), ab.range, false, ab.vis_flags,
                ab.vis_bans)
...
function scripts.tower_royal_archers_pow_rapacious_hunter_eagle.update(this, store)
    ...
    while true do
        ...
        if not store.entities[target.id] or target.health.dead or far_from_tower or not target_still_valid or
            ...
            local _, targets = U.find_foremost_enemy_in_range_filter_off(tpos(this.owner), tamer_attack.range, false,
...
function scripts.tower_arcane_wizard5.update(this, store)
    ...
    local function find_target(aa)
        local target = U.detect_foremost_enemy_in_range_filter_on(tpos(this), a.range, aa.vis_flags, aa.vis_bans,
            function(e)
        ...
        if this.tower.blocked then
            ...
            if ready_to_use_power(pow_d, ad, store, tw.cooldown_factor) then
                ...
                if not enemy then
                    ...
                    local _, enemies = U.find_foremost_enemy_in_range_filter_on(tpos(this), a.range, ad.node_prediction,
            ...
            if store.tick_ts - ar.ts > this.tower.long_idle_cooldown then
                local soffset = this.shooter_offset
                local an, af, ai = animation_name_facing_point(this, "idle", this.tower.long_idle_pos, shooter_sid,
                    soffset)
    ...
    while true do
        ...
        if pow_s.level > 0 then
            if sting_missiles_ready then
                ...
                sting_missiles_ready = store.tick_ts - sting_missiles_ts > pow_s.cooldown[pow_s.level] *
                                           this.tower.cooldown_factor
            end
        end
...
function scripts.soldier_tower_rocket_gunners.update(this, store)
    ...
    local function adjust_position_reference()
        ...
        if this.ranged.attacks[3] then
            this.ranged.attacks[3].bullet_start_offset = {v(this.ranged.attacks[3].bullet_start_offset_relative.x,
                this.render.sprites[1].offset.y + this.ranged.attacks[3].bullet_start_offset_relative.y)}
        end

        if this.current_mode == MODE_GROUND then
            local new_height = U.ease_value(this.health_bar.offset.y, this.health_bar.y_offset, store.tick_length * 10,
                "linear")

            this.health_bar.offset.y = new_height
        else
            local new_height = U.ease_value(this.health_bar.offset.y, this.flight_height + this.health_bar.y_offset,
                store.tick_length * 10, "linear")
    ...
    local function adjust_height()
        ...
        local new_height = U.ease_value(this.render.sprites[1].offset.y, height_dest, store.tick_length * strength,
            easing)
            ...
            if p.changed then
                ...
                if p == this.powers.sting_missiles then
                    ...
                    this.ranged.attacks[3].filter_fn = function(e)
                        return
                            e.health and e.health.hp <= e.health.hp_max * p.kill_hp_factor[p.level] and e.health.hp >=
                                500
                    end
                end
            end
        ...
        if this.ranged and not this.ranged.range_while_blocking then
            if this.ranged.attacks[2] and this.ranged.attacks[2].bullet_start_offset then
                for _, start_offset in ipairs(this.ranged.attacks[2].bullet_start_offset) do
                    start_offset.y = this.render.sprites[1].offset.y +
                                         this.ranged.attacks[2].bullet_start_offset_relative.y
...
function scripts.bullet_soldier_tower_rocket_gunners_sting_missiles.update(this, store)
    ...
    local function fly_to_pos(target_pos)
        ...
        while V.len(dx, dy) > 20 do
            ...
            ps.particle_system.emit_offset.x, ps.particle_system.emit_offset.y =
                V.rotate(this.render.sprites[1].r, ps.emit_offset_relative.x, ps.emit_offset_relative.y)
    ...
    if not target or target.health.dead then
        local new_target = U.detect_foremost_enemy_in_range_filter_on(soldier_floor_pos, attack.max_range,
    ...
    while true do
        target = store.entities[b.target_id]
        if not target or target.health.dead then
            target = U.detect_foremost_enemy_in_range_filter_on(soldier_floor_pos, attack.max_range, attack.vis_flags,
                attack.vis_bans, attack.filter_fn)
        end
...
function scripts.tower_flamespitter.update(this, store)
    ...
    local function shoot_bomb(enemy, dest)
        ...
        b.pos.x, b.pos.y = this.pos.x + attack_bomb.bullet_start_offset.x,
            this.pos.y + attack_bomb.bullet_start_offset.y
        b.bullet.from = vclone(b.pos)
        b.bullet.to = dest
    ...
    a_name, a_flip, angle_idx = animation_name_facing_angle_flamespitter("idle", this.tower_upgrade_persistent_data
        .current_angle)
    ...
    while true do
        if this.tower.blocked then
            ...
            if not this.render.sprites[this.render.sid_stove_fire].hidden and
                animation_finished(this, this.render.sid_stove_fire) then
            ...
            if ready_to_attack(attack_basic, store, tw.cooldown_factor) then
                ...
                a_name, a_flip, angle_idx = animation_name_facing_point_flamespitter("idle", pred_pos,
                    this.tower_top_offset)
                ...
                while store.tick_ts - fire_ts < attack_basic.duration * tw.cooldown_factor do
                    ...
                    if store.tick_ts - fire_cycle_ts >= attack_basic.cycle_time * tw.cooldown_factor then
                        ...
                        local aura_targets = U.find_enemies_in_range_filter_on(tpos, this.attacks.range,
                            attack_basic.vis_flags, attack_basic.vis_bans, function(v)
                        ...
                        if aura_targets then
                            for _, aura_target in ipairs(aura_targets) do
                                ...
                                if up then
                                    ...
                                    d.value = (random(attack_basic.damage_min, attack_basic.damage_max)) *
                                                  tw.damage_factor
                                end
...
function scripts.controller_tower_flamespitter_column.update(this, store)
    ...
    local enemies = U.find_enemies_between_range_filter_off(this.dest, this.radius_in, this.radius_out, this.vis_flags,
        this.vis_bans)
    ...
    while true do
        if this.tower.blocked then
            ...
            if idle_flip_cd < store.tick_ts - idle_flip_last_ts then
                U.animation_start(this, "idle", not this.render.sprites[this.sid_viking].flip_x, store.tick_ts, true,
                    this.sid_viking)
            ...
            if ready_to_use_power(pow_b, bba, store, tw.cooldown_factor) then
                ...
                if not targets or #targets < bba.min_targets then
                    ...
                    local enemy, enemies, pred_pos = U.find_foremost_enemy_with_max_coverage_in_range_filter_off(tpos,
                        a.range, bba.node_prediction, bba.vis_flags, bba.vis_bans, E:get_template(
                            "bullet_tower_barrel_skill_barrel").bullet.damage_radius)
            ...
            if ready_to_attack(ba, store, tw.cooldown_factor) then
                ...
                if not enemy then
                    ...
                    local enemy, _, pred_pos = U.find_foremost_enemy_with_max_coverage_in_range_filter_off(tpos,
                        a.range, ba.node_prediction, ba.vis_flags, ba.vis_bans, E:get_template(
                            "bullet_tower_barrel_lvl4").bullet.damage_radius)
...
function scripts.aura_bullet_tower_barrel_skill_barrel.update(this, store, script)
    ...
    local targets = U.find_enemies_in_range_filter_off(this.pos, this.explosion_damage_radius, this.explosion_vis_flags,
        this.explosion_vis_bans)
    if targets then
        ...
        if r.new then
            ...
            while not this.motion.arrived do
                ...
                if r._first_time then
                    ...
                    local target = U.find_foremost_enemy(store.entities, r.center, 0, this.melee.range, false, F_BLOCK,
    ...
    local function check_tower_skill_upgrade()
        if tower then
            ...
            if current_level ~= warrior_level then
                ...
                SU.damage_inc(this,
                    (a1.damage_min_config[warrior_level] + a1.damage_max_config[warrior_level] -
                        a1.damage_min_config[current_level] - a1.damage_max_config[current_level]) / 2)
