return {
	groups = {
		BlackburnSounds = {
			append = true,
			sounds = {"HWZombieAmbient", "kr4_tower_wickedsisters_attack_v1", "CBSpectralKnight"}
		},
		ElvesCommonSounds = {
			append = true,
			files = {"assassin_gold.ogg", "Viking_Transform.ogg"}
		},
		ElvesDrizzt = {
			append = true,
			files = {"kro_sfx_bitteringrancor_drizzt.ogg"}
		},
		ElvesHeroAlleria = {
			append = true,
			files = {"Aleria_special_arrow.ogg", "Aleria_wildcat_hit.ogg"}
		},
		ElvesHeroArivan = {
			append = true,
			files = {"archmage_attack.ogg"}
		},
		ElvesLevelHRSounds = {
			append = true,
			files = {"GalhadriansBastion-01c.ogg", "GalhadriansBastion-02c.ogg"}
		},
		ElvesLevelNineSounds = {
			append = true,
			files = {"mermaid.ogg"}
		},
		ElvesLevelThreeSounds = {
			append = true,
			files = {"mermaid.ogg"}
		},
		HeroCrab = {
			append = true,
			sounds = {"HeroGiantExplosionRock"}
		},
		HeroDracolich = {
			append = true,
			sounds = {"HeroDragonAttackHit"}
		},
		HeroMirage = {
			append = true,
			sounds = {"HeroMirageDeath"}
		},
		HeroViking = {
			append = true,
			sounds = {"HeroVikingTaunt"}
		},
		MushroomSounds = {
			append = true,
			sounds = {"MushroomPoof"}
		},
		common = {
			append = true,
			sounds = {"MeleeSword", "swamp_thing_bomb_shot", "swamp_thing_bomb_explosion", "DeathEplosionShort", "DeathEplosionShortA", "juggernaut_punch", "TenShiTauntSelect"}
		},
		hero_10yr = {
			sounds = {
				"TenShiTaunt",
				"TenShiTauntBuffed",
				"TenShiTauntIntro",
				"TenShiTauntSelect",
				"TenShiRespawn",
				"TenShiTransformToBuffed",
				"TenShiTransformToNormal",
				"TenShiAttack1",
				"TenShiAttack2",
				"TenShiRainOfFireStart",
				"TenShiRainOfFireEnd",
				"TenShiBuffedSpinAttack",
				"TenShiBuffedBombAttack",
				"TenShiBuffedBombAttackLong",
				"TenShiTransformToBuffedSfx",
				"TenShiTransformToNormalSfx",
				"TenShiDeathSfx",
				"TenShiTeleportSfx"
			}
		},
		hero_alien = {
			alias = "HeroAlien"
		},
		hero_alleria = {
			alias = "HeroArcher"
		},
		hero_alric = {
			alias = "HeroAlric"
		},
		hero_arivan = {
			alias = "ElvesHeroArivan"
		},
		hero_baby_malik = {
			alias = "ElvesHeroBabyMalik"
		},
		hero_beastmaster = {
			alias = "HeroBeastMaster"
		},
		hero_bolin = {
			alias = "HeroRifleman"
		},
		hero_bolverk = {
			alias = "ElvesHeroBolverk"
		},
		hero_bravebark = {
			alias = "ElvesHeroForestElemental"
		},
		hero_bruce = {
			alias = "ElvesHeroBruce"
		},
		hero_catha = {
			alias = "ElvesHeroCatha"
		},
		hero_crab = {
			alias = "HeroCrab"
		},
		hero_denas = {
			alias = "HeroDenas"
		},
		hero_dracolich = {
			alias = "HeroDracolich"
		},
		hero_dragon = {
			alias = "HeroDragon"
		},
		hero_durax = {
			alias = "ElvesHeroDurax"
		},
		hero_dwarf = {
			alias = "HeroDwarf"
		},
		hero_elora = {
			alias = "HeroFrost"
		},
		hero_elves_archer = {
			alias = "ElvesHeroEridan"
		},
		hero_elves_denas = {
			alias = "ElvesHeroDenas"
		},
		hero_faustus = {
			alias = "ElvesHeroFaustus"
		},
		hero_gerald = {
			alias = "HeroPaladin"
		},
		hero_giant = {
			alias = "HeroGiant"
		},
		hero_hacksaw = {
			alias = "HeroHacksaw"
		},
		hero_ignus = {
			alias = "HeroRainOfFire"
		},
		hero_ingvar = {
			alias = "HeroViking"
		},
		hero_lilith = {
			alias = "ElvesHeroLilith"
		},
		hero_lynn = {
			alias = "ElvesHeroLynn"
		},
		hero_magnus = {
			alias = "HeroMage"
		},
		hero_malik = {
			alias = "HeroReinforcement"
		},
		hero_minotaur = {
			alias = "HeroMinotaur"
		},
		hero_mirage = {
			alias = "HeroMirage"
		},
		hero_monk = {
			alias = "HeroMonk"
		},
		hero_monkey_god = {
			alias = "HeroMonkeyGod"
		},
		hero_oni = {
			alias = "HeroSamurai"
		},
		hero_phoenix = {
			alias = "ElvesHeroPhoenix"
		},
		hero_pirate = {
			alias = "HeroPirate"
		},
		hero_priest = {
			alias = "HeroPriest"
		},
		hero_rag = {
			alias = "ElvesHeroRag"
		},
		hero_regson = {
			alias = "ElvesHeroEldritch"
		},
		hero_thor = {
			alias = "HeroThor"
		},
		hero_vampiress = {
			alias = "HWVampiress"
		},
		hero_van_helsing = {
			alias = "HeroVanHelsing"
		},
		hero_veznan = {
			alias = "ElvesHeroVeznan"
		},
		hero_voodoo_witch = {
			alias = "HeroVoodooWitch"
		},
		hero_wilbur = {
			alias = "ElvesHeroGyro"
		},
		hero_wizard = {
			alias = "HeroWizard"
		},
		hero_xin = {
			alias = "ElvesHeroXin"
		},
		music_screen_credits = {
			sounds = {"MusicEndCredits"}
		},
		music_screen_kr1_end = {
			sounds = {"MusicEndCredits", "MusicSuspense"}
		},
		music_screen_kr2_end = {
			sounds = {"MusicEndVictory", "MusicEndCredits", "MusicSuspense"}
		},
		music_screen_kr3_end = {
			sounds = {"MusicEndVictory", "MusicSuspense", "MusicCredits"}
		},
		music_screen_map = {
			sounds = {"MusicMap1", "MusicMap2", "MusicMap3", "MusicMap5"}
		},
		music_screen_slots = {
			sounds = {"MusicMainMenu"}
		},
		music_stage01 = {
			sounds = {"MusicBattlePrep_01", "MusicBattle_01"}
		},
		music_stage02 = {
			sounds = {"MusicBattlePrep_02", "MusicBattle_02"}
		},
		music_stage03 = {
			sounds = {"MusicBattlePrep_03", "MusicBattle_03"}
		},
		music_stage04 = {
			sounds = {"MusicBattlePrep_04", "MusicBattle_04"}
		},
		music_stage05 = {
			sounds = {"MusicBattlePrep_05", "MusicBattle_05"}
		},
		music_stage06 = {
			sounds = {"MusicBattlePrep_06", "MusicBattle_06", "MusicBossFight"}
		},
		music_stage07 = {
			sounds = {"MusicBattlePrep_07", "MusicBattle_07"}
		},
		music_stage08 = {
			sounds = {"MusicBattlePrep_08", "MusicBattle_08"}
		},
		music_stage09 = {
			sounds = {"MusicBattlePrep_09", "MusicBattle_09", "MusicBossFight"}
		},
		music_stage10 = {
			sounds = {"MusicBattlePrep_10", "MusicBattle_10"}
		},
		music_stage1000 = {
			sounds = {"MusicBattlePrep_01", "MusicBattle_01"}
		},
		music_stage11 = {
			sounds = {"MusicBattlePrep_11", "MusicBattle_11"}
		},
		music_stage12 = {
			sounds = {"MusicBattlePrep_12", "MusicBattle_12", "MusicBossFight"}
		},
		music_stage13 = {
			sounds = {"MusicBattlePrep_13", "MusicBattle_13", "MusicBossFight"}
		},
		music_stage14 = {
			sounds = {"MusicBattlePrep_14", "MusicBattle_14", "MusicBossFight"}
		},
		music_stage15 = {
			sounds = {"MusicBattlePrep_15", "MusicBattle_15", "MusicBossFight"}
		},
		music_stage16 = {
			sounds = {"MusicBattlePrep_16", "MusicBattle_16"}
		},
		music_stage17 = {
			sounds = {"MusicBattlePrep_17", "MusicBattle_17", "MusicBossFight"}
		},
		music_stage18 = {
			sounds = {"MusicBattlePrep_18", "MusicBattle_18"}
		},
		music_stage19 = {
			sounds = {"MusicBattlePrep_19", "MusicBattle_19", "MusicBossFight"}
		},
		music_stage20 = {
			sounds = {"MusicBattlePrep_20", "MusicBattle_20"}
		},
		music_stage21 = {
			sounds = {"MusicBattlePrep_21", "MusicBattle_21", "MusicBossFight"}
		},
		music_stage22 = {
			sounds = {"MusicBattlePrep_22", "MusicBattle_22", "MusicBossFight"}
		},
		music_stage23 = {
			sounds = {"MusicBattlePrep_23", "MusicBattle_23"}
		},
		music_stage24 = {
			sounds = {"MusicBattlePrep_24", "MusicBattle_24"}
		},
		music_stage25 = {
			sounds = {"MusicBattlePrep_25", "MusicBattle_25"}
		},
		music_stage26 = {
			sounds = {"MusicBattlePrep_26", "MusicBattle_26", "MusicBossFight"}
		},
		music_stage27 = {
			sounds = {"MusicBattlePrep_27", "MusicBattle_27", "MusicBossPreFight"}
		},
		music_stage28 = {
			sounds = {"MusicBattlePrep_28", "MusicBattle_28"}
		},
		music_stage29 = {
			sounds = {"MusicBattlePrep_29", "MusicBattle_29"}
		},
		music_stage30 = {
			sounds = {"MusicBattlePrep_30", "MusicBattle_30"}
		},
		music_stage31 = {
			sounds = {"MusicBattlePrep_31", "MusicBattle_31"}
		},
		music_stage32 = {
			sounds = {"MusicBattlePrep_32", "MusicBattle_32", "MusicBossFight"}
		},
		music_stage33 = {
			sounds = {"MusicBattlePrep_33", "MusicBattle_33"}
		},
		music_stage34 = {
			sounds = {"MusicBattlePrep_34", "MusicBattle_34"}
		},
		music_stage35 = {
			sounds = {"MusicBattlePrep_35", "MusicBattle_35"}
		},
		music_stage36 = {
			sounds = {"MusicBattlePrep_36", "MusicBattle_36"}
		},
		music_stage37 = {
			sounds = {"MusicBattlePrep_37", "MusicBattle_37", "MusicBossPreFight", "MusicBossFight"}
		},
		music_stage38 = {
			sounds = {"MusicBattlePrep_38", "MusicBattle_38"}
		},
		music_stage39 = {
			sounds = {"MusicBattlePrep_39", "MusicBattle_39"}
		},
		music_stage40 = {
			sounds = {"MusicBattlePrep_40", "MusicBattle_40"}
		},
		music_stage41 = {
			sounds = {"MusicBattlePrep_41", "MusicBattle_41", "MusicBossPreFight", "MusicBossFight"}
		},
		music_stage42 = {
			sounds = {"MusicBattlePrep_42", "MusicBattle_42"}
		},
		music_stage43 = {
			sounds = {"MusicBattlePrep_43", "MusicBattle_43"}
		},
		music_stage44 = {
			sounds = {"MusicBattlePrep_44", "MusicBattle_44", "MusicBossPreFightEnd", "MusicBossFight"}
		},
		music_stage45 = {
			sounds = {"MusicBattlePrep_45", "MusicBattle_45", "MusicHalloweenMoon"}
		},
		music_stage46 = {
			sounds = {"MusicBattlePrep_46", "MusicBattle_46", "MusicHalloweenMoon"}
		},
		music_stage47 = {
			sounds = {"MusicBattlePrep_47", "MusicBattle_47", "MusicHalloweenMoon", "MusicBossFight"}
		},
		music_halloween_moon = {
			sounds = {"MusicHalloweenMoon"}
		},
		music_stage48 = {
			sounds = {"MusicBattlePrep_48", "MusicBattle_48", "MusicBossFight"}
		},
		music_stage49 = {
			sounds = {"MusicBattlePrep_49", "MusicBattle_49"}
		},
		music_stage50 = {
			sounds = {"MusicBattlePrep_50", "MusicBattle_50"}
		},
		music_stage51 = {
			sounds = {"MusicBattlePrep_51", "MusicBattle_51"}
		},
		music_stage52 = {
			sounds = {"MusicBattlePrep_52", "MusicBattle_52"}
		},
		music_stage53 = {
			sounds = {"MusicBattlePrep_53", "MusicBattle_53"}
		},
		music_stage54 = {
			sounds = {"MusicBattlePrep_54", "MusicBattle_54"}
		},
		music_stage55 = {
			sounds = {"MusicBattlePrep_55", "MusicBattle_55"}
		},
		music_stage56 = {
			sounds = {"MusicBattlePrep_56", "MusicBattle_56"}
		},
		music_stage57 = {
			sounds = {"MusicBattlePrep_57", "MusicBattle_57"}
		},
		music_stage58 = {
			sounds = {"MusicBattlePrep_58", "MusicBattle_58"}
		},
		music_stage59 = {
			sounds = {"MusicBattlePrep_59", "MusicBattle_59"}
		},
		music_stage60 = {
			sounds = {"MusicBattlePrep_60", "MusicBattle_60"}
		},
		music_stage61 = {
			sounds = {"MusicBattlePrep_61", "MusicBattle_61"}
		},
		music_stage62 = {
			sounds = {"MusicBattlePrep_62", "MusicBattle_62"}
		},
		music_stage63 = {
			sounds = {"MusicBattlePrep_63", "MusicBattle_63", "MusicBossPreFight63", "MusicBossFight"}
		},
		music_stage64 = {
			sounds = {"MusicBattlePrep_64", "MusicBattle_64"}
		},
		music_stage65 = {
			sounds = {"MusicBattlePrep_65", "MusicBattle_65"}
		},
		music_stage66 = {
			sounds = {"MusicBattlePrep_66", "MusicBattle_66"}
		},
		music_stage67 = {
			sounds = {"MusicBattlePrep_67", "MusicBattle_67"}
		},
		music_stage68 = {
			sounds = {"MusicBattlePrep_68", "MusicBattle_68"}
		},
		music_stage69 = {
			sounds = {"MusicBattlePrep_69", "MusicBattle_69"}
		},
		music_stage70 = {
			sounds = {"MusicBattlePrep_70", "MusicBattle_70"}
		},
		music_stage101 = {
			sounds = {"MusicBattlePrep_101", "MusicBattle_101"}
		},
		music_stage102 = {
			sounds = {"MusicBattlePrep_102", "MusicBattle_102"}
		},
		music_stage103 = {
			sounds = {"MusicBattlePrep_103", "MusicBattle_103"}
		},
		music_stage104 = {
			sounds = {"MusicBattlePrep_104", "MusicBattle_104"}
		},
		music_stage105 = {
			sounds = {"MusicBattlePrep_105", "MusicBattle_105"}
		},
		music_stage106 = {
			sounds = {"MusicBattlePrep_106", "MusicBattle_106", "MusicBossFight_106"}
		},
		music_stage107 = {
			sounds = {"MusicBattlePrep_107", "MusicBattle_107"}
		},
		music_stage108 = {
			sounds = {"MusicBattlePrep_108", "MusicBattle_108"}
		},
		music_stage109 = {
			sounds = {"MusicBattlePrep_109", "MusicBattle_109"}
		},
		music_stage110 = {
			sounds = {"MusicBattlePrep_110", "MusicBattle_110"}
		},
		music_stage111 = {
			sounds = {"MusicBattlePrep_111", "MusicBattle_111", "MusicBossFight_111"}
		},
		music_stage112 = {
			sounds = {"MusicBattlePrep_112", "MusicBattle_112"}
		},
		music_stage113 = {
			sounds = {"MusicBattlePrep_113", "MusicBattle_113"}
		},
		music_stage114 = {
			sounds = {"MusicBattlePrep_114", "MusicBattle_114"}
		},
		music_stage115 = {
			sounds = {"MusicBattlePrep_115", "MusicBattle_115", "MusicBossFight_115"}
		},
		music_stage116 = {
			sounds = {"MusicBattlePrep_116", "MusicBattle_116", "MusicBossFight_116"}
		},
		music_stage117 = {
			sounds = {"MusicBattlePrep_117", "MusicBattle_117"}
		},
		music_stage118 = {
			sounds = {"MusicBattlePrep_118", "MusicBattle_118"}
		},
		music_stage119 = {
			sounds = {"MusicBattlePrep_119", "MusicBattle_119", "MusicBossFight_119"}
		},
		music_stage120 = {
			sounds = {"MusicBattlePrep_120", "MusicBattle_120"}
		},
		music_stage121 = {
			sounds = {"MusicBattlePrep_121", "MusicBattle_121"}
		},
		music_stage122 = {
			sounds = {"MusicBattlePrep_122", "MusicBattle_122", "MusicBossFight_122"}
		},
		music_stage123 = {
			sounds = {"MusicBattlePrep_123", "MusicBattle_123"}
		},
		music_stage124 = {
			sounds = {"MusicBattlePrep_124", "MusicBattle_124"}
		},
		music_stage125 = {
			sounds = {"MusicBattlePrep_125", "MusicBattle_125"}
		},
		music_stage126 = {
			sounds = {"MusicBattlePrep_126", "MusicBattle_126"}
		},
		music_stage127 = {
			sounds = {"MusicBattlePrep_127", "MusicBattle_127", "MusicBossFight_127"}
		},
		music_stage128 = {
			sounds = {"MusicBattlePrep_128", "MusicBattle_128"}
		},
		music_stage129 = {
			sounds = {"MusicBattlePrep_129", "MusicBattle_129"}
		},
		music_stage130 = {
			sounds = {"MusicBattlePrep_130", "MusicBattle_130", "MusicBossFight_130"}
		},
		music_stage131 = {
			sounds = {"MusicBattlePrep_131", "MusicBattle_131"}
		},
		music_stage132 = {
			sounds = {"MusicBattlePrep_132", "MusicBattle_132"}
		},
		music_stage133 = {
			sounds = {"MusicBattlePrep_133", "MusicBattle_133"}
		},
		music_stage134 = {
			sounds = {"MusicBattlePrep_134", "MusicBattle_134"}
		},
		music_stage135 = {
			sounds = {"MusicBattlePrep_135", "MusicBattle_135", "MusicBossFight_135"}
		},
		music_stage136 = {
			sounds = {"MusicBattlePrep_136", "MusicBattle_136"}
		},
		music_stage137 = {
			sounds = {"MusicBattlePrep_137", "MusicBattle_137"}
		},
		music_stage138 = {
			sounds = {"MusicBattlePrep_138", "MusicBattle_138"}
		},
		music_stage139 = {
			sounds = {"MusicBattlePrep_139", "MusicBattle_139"}
		},
		music_stage140 = {
			sounds = {"MusicBattlePrep_140", "MusicBattle_140", "MusicBossFight_140"}
		},
		music_stage1001 = {
			sounds = {"MusicBattlePrep_1001", "MusicBattle_1001", "MusicBossFight"}
		},
		music_stage2000 = {
			sounds = {"MusicBattlePrep_2000", "MusicBattle_2000", "MusicBossFight"}
		},
		music_stage2001 = {
			sounds = {"MusicBattlePrep_2001", "MusicBattle_2001"}
		},
		music_stage2002 = {
			sounds = {"MusicBattlePrep_2002", "MusicBattle_2002", "MusicBossFight"}
		},
		music_stage2003 = {
			sounds = {"MusicBattlePrep_2003", "MusicBattle_2003", "MusicBossFight"}
		},
		music_stage2004 = {
			sounds = {"MusicBattlePrep_2004", "MusicBattle_2004"}
		},
		music_stage3000 = {
			sounds = {"MusicBattlePrep_3000", "MusicBattle_3000"}
		}
	},
	sounds = {
		ArchmageTwisterTravel = {
			files = {"archmage_twister.ogg"},
			gain = 0.3,
			loop = true,
			ref_counted = true,
			source_group = "REFCOUNTED"
		},
		ArrowSound = {
			files = {"Sound_ArrowRelease2.ogg", "Sound_ArrowRelease3.ogg"},
			gain = {0.65, 0.8},
			loop = false,
			source_group = "BULLETS"
		},
		AssassinGold = {
			files = {"assassin_gold.ogg"},
			gain = 0.6,
			loop = false,
			source_group = "SPECIALS"
		},
		AxeSound = {
			files = {"Sound_BattleAxe.ogg"},
			gain = {0.3, 0.5},
			loop = false,
			source_group = "BULLETS"
		},
		BossMonkeyMonkeysScreams = {
			every = 4,
			files = {"boss_mono_chimps_1.ogg", "boss_mono_chimps_2.ogg", "boss_mono_chimps_3.ogg"},
			gain = 1,
			loop = false,
			mode = "random",
			source_group = "SFX"
		},
		CBSpectralKnight = {
			files = {"kr_sfx_cb_spectralknight[horn+organ].ogg"},
			gain = 0.6,
			loop = false,
			source_group = "SFX"
		},
		DeathBig = {
			files = {"Sound_EnemyBigDead.ogg"},
			gain = {0.6, 0.9},
			loop = false,
			source_group = "DEATH"
		},
		DeathEplosionShort = {
			files = {"Sound_EnemyExplode1Short.ogg"},
			gain = 0.6,
			loop = false,
			source_group = "DEATH"
		},
		DeathEplosionShortA = {
			files = {"Sound_EnemyExplode1ShortA.ogg"},
			gain = 0.2,
			loop = false,
			source_group = "DEATH"
		},
		DeathSkeleton = {
			files = {"Sound_EnemySkeletonBreak2.ogg"},
			gain = {0.6, 0.9},
			loop = false,
			source_group = "DEATH"
		},
		DwarfArcherTaunt1 = {
			files = {"dwarfArcher_taunt_1.ogg"},
			gain = 1,
			loop = false,
			source_group = "TAUNTS"
		},
		DwarfArcherTaunt2 = {
			files = {"dwarfArcher_taunt_2.ogg"},
			gain = 1,
			loop = false,
			source_group = "TAUNTS"
		},
		DwarfHeroTauntIntro = {
			files = {"Level_up.ogg"},
			gain = 1,
			loop = false,
			source_group = "TAUNTS"
		},
		ElvenWoodsAmbienceEndlessSound = {
			files = {"kre_sfx_ambience_singlebirdandstream.ogg", "kro_sfx_ambience_wardrums[op1].ogg", "kro_sfx_ambience_wardrums[op2].ogg", "kro_sfx_ambience_wardrums[op3].ogg"},
			gain = 0.3,
			loop = false,
			mode = "random",
			source_group = "SFX"
		},
		ElvenWoodsAmbienceSound = {
			files = {"kre_sfx_ambience_singlebirdandstream.ogg"},
			gain = 0.3,
			loop = false,
			mode = "random",
			source_group = "SFX"
		},
		ElvesAlleriaCatHit = {
			files = {"Aleria_wildcat_hit.ogg"},
			gain = 1,
			loop = false,
			source_group = "BULLETS"
		},
		ElvesBajNimenBossShadowImpact = {
			files = {"kro_sfx_bitteringrancor_boss-shadowstorm[impact].ogg"},
			gain = 0.3,
			ignore = 0.25,
			loop = false,
			source_group = "BULLETS"
		},
		ElvesBajNimenBossShadowTravel = {
			files = {"kro_sfx_bitteringrancor_boss-shadowstorm[travel].ogg"},
			gain = 0.5,
			ignore = 0.25,
			loop = false,
			source_group = "BULLETS"
		},
		ElvesCreepHoplite = {
			files = {"kro_sfx_hoplite_summon_v3[singrowl].ogg"},
			gain = 0.5,
			loop = false,
			source_group = "SFX"
		},
		ElvesDrizztUnsheathe = {
			files = {"kro_sfx_bitteringrancor_drizzt.ogg"},
			gain = 0.5,
			loop = false,
			source_group = "SFX"
		},
		ElvesFinalBossCastSpell = {
			files = {"kro_sfx_spidergoddess_castspell.ogg", "kro_sfx_spidergoddess_castspellwithlaughter.ogg"},
			gain = 1,
			loop = false,
			mode = "random",
			source_group = "SFX"
		},
		ElvesFinalBossWebground = {
			files = {"kro_sfx_mactans_webground[conhissing].ogg"},
			gain = 1,
			interruptible = true,
			loop = false,
			source_group = "SFX"
		},
		ElvesFinalBossWebspin = {
			files = {"kro_sfx_mactans_webspin[towers].ogg"},
			gain = 1,
			interruptible = true,
			loop = false,
			source_group = "SFX"
		},
		ElvesGoldCoin = {
			files = {"assassin_gold.ogg"},
			gain = 0.6,
			loop = false,
			source_group = "SPECIALS"
		},
		ElvesHeroAlleriaShoot = {
			files = {"Aleria_special_arrow.ogg"},
			gain = 1,
			loop = false,
			source_group = "BULLETS"
		},
		ElvesHeroArivanRegularRay = {
			files = {"archmage_attack.ogg"},
			gain = 1,
			loop = false,
			source_group = "SPECIALS"
		},
		ElvesHeroGyroBombsMarch = {
			files = {"kro_sfx_wilbur_bombsmarch[op1].ogg"},
			gain = 0.8,
			loop = true,
			ref_counted = true,
			source_group = "SFX"
		},
		ElvesTowerBastionInsertTaunt = {
			files = {"GalhadriansBastion-01c.ogg"},
			gain = 1,
			loop = false,
			source_group = "TAUNTS"
		},
		ElvesTowerBastionRazorEdge = {
			files = {"GalhadriansBastion-02c.ogg"},
			gain = 1,
			loop = false,
			source_group = "TAUNTS"
		},
		ElvesWaterfallMid = {
			files = {"mermaid.ogg"},
			gain = 0.6,
			loop = false,
			source_group = "SFX"
		},
		EnemyBlackburnWitchDeath = {
			files = {"kr_sfx_cb_witchdeath.ogg"},
			gain = 0.2,
			loop = false,
			source_group = "SFX"
		},
		FaerieGroveAmbienceSound = {
			files = {"kro_sfx_ambience_windandowls1.ogg"},
			gain = 0.8,
			loop = false,
			mode = "random",
			source_group = "SFX"
		},
		FaerieGroveAmbienceTenElevenSound = {
			files = {"kro_sfx_ambience_windandowls1.ogg", "kro_sfx_ambience_branchesandfaery1.ogg", "kro_sfx_ambience_branchesandfaery2.ogg"},
			gain = 0.8,
			loop = false,
			mode = "random",
			source_group = "SFX"
		},
		GoldCoin = {
			files = {"assassin_gold.ogg"},
			gain = 0.6,
			loop = false,
			source_group = "SPECIALS"
		},
		HWFrankensteinTaunt = {
			files = {"FrankyTower-03c.ogg", "FrankyTower-04a.ogg"},
			gain = 1,
			loop = false,
			mode = "random",
			source_group = "TAUNTS"
		},
		HWFrankensteinUpgradeFrankenstein = {
			files = {"FrankyTower-02f.ogg"},
			gain = 0.8,
			loop = false,
			source_group = "TAUNTS"
		},
		HWFrankensteinUpgradeLightning = {
			files = {"FrankyTower-01d.ogg"},
			gain = 0.8,
			loop = false,
			source_group = "TAUNTS"
		},
		HWVampiressTaunt = {
			files = {"Vampiress-01c.ogg", "Vampiress-02c.ogg", "Vampiress-03c.ogg"},
			gain = 1,
			loop = false,
			mode = "random",
			source_group = "TAUNTS"
		},
		HWVampiressTauntIntro = {
			files = {"Level_up.ogg"},
			gain = 1,
			loop = false,
			source_group = "TAUNTS"
		},
		HWVampiressTauntSelect = {
			files = {"Vampiress-01c.ogg"},
			gain = 1,
			loop = false,
			source_group = "TAUNTS"
		},
		HWZombieAmbient = {
			files = {"canibal_zombie_one.ogg", "canibal_zombie_two.ogg", "canibal_zombie_three.ogg"},
			gain = 0.6,
			ignore = 3,
			loop = false,
			mode = "random",
			source_group = "SFX"
		},
		HeroMirageDeath = {
			files = {"assassinHero_taunt_death.ogg"},
			gain = 0.6,
			loop = false,
			source_group = "TAUNTS"
		},
		HeroMonkMultihitScream = {
			files = {"Hero_Monk_Multihit_scream_1.ogg", "Hero_Monk_Multihit_scream_2.ogg"},
			gain = 1,
			loop = false,
			mode = "random",
			source_group = "SPECIALS"
		},
		HeroThorDeath = {
			files = {"Thor_05c.ogg", "KR_sfx_thor_muerte.ogg"},
			gain = 1,
			loop = false,
			mode = "concurrent",
			source_group = "TAUNTS"
		},
		HeroVikingTaunt = {
			files = {"Viking-01b.ogg", "Viking-02c.ogg", "Viking-03b.ogg", "Viking-04c.ogg"},
			gain = 1,
			ignore = 1,
			loop = false,
			mode = "sequence",
			source_group = "TAUNTS"
		},
		HulkingRageAmbienceSound = {
			files = {"kre_sfx_ambience_singlebirdandstream.ogg", "kro_sfx_hulkingrage_mining_v2[op1].ogg", "kro_sfx_hulkingrage_mining_v2[op2].ogg"},
			gain = 0.3,
			loop = false,
			mode = "random",
			source_group = "SFX"
		},
		MechWalk = {
			files = {"mecha_walk.ogg"},
			gain = 0.2,
			loop = true,
			source_group = "SPECIALS"
		},
		MeleeSword = {
			files = {"Sound_SoldiersFighting-01.ogg", "Sound_SoldiersFighting-02.ogg", "Sound_SoldiersFighting-03.ogg", "Sound_SoldiersFighting-04.ogg", "Sound_SoldiersFighting-05.ogg"},
			gain = 0.2,
			ignore = 0.45,
			loop = false,
			mode = "sequence",
			source_group = "SWORDS"
		},
		MetropolisAmbienceSound = {
			files = {"kro_sfx_ambience_winds[op1].ogg", "kro_sfx_ambience_winds[op2].ogg"},
			gain = 0.3,
			loop = false,
			mode = "random",
			source_group = "SFX"
		},
		MetropolisAmbienceSoundFourteenFifteen = {
			files = {"kro_sfx_ambience_winds[op1].ogg", "kro_sfx_ambience_winds[op2].ogg", "kro_sfx_ambience_spiders_v2[op1].ogg"},
			gain = 0.3,
			loop = false,
			mode = "random",
			source_group = "SFX"
		},
		MushroomPoof = {
			files = {"KR_sfx_mushroomcreep_muerte_edit_vanzen.ogg"},
			gain = {0.2, 0.3},
			loop = false,
			source_group = "SFX"
		},
		MusicBattlePrep_01 = {
			files = {"MusicBattlePreparationsForest.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_02 = {
			files = {"MusicBattlePreparationsForest.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_03 = {
			files = {"MusicBattlePreparationsForest.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_04 = {
			files = {"MusicBattlePreparationsForest.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_05 = {
			files = {"MusicBattlePreparationsForest.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_06 = {
			files = {"MusicBattlePreparationsForest.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_07 = {
			files = {"MusicBattlePreparationsMountain.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_08 = {
			files = {"MusicBattlePreparationsMountain.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_09 = {
			files = {"MusicBattlePreparationsMountain.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_10 = {
			files = {"MusicBattlePreparationsWastelands.ogg"},
			gain = 0.9,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_1000 = {
			files = {"MusicBattlePreparationsForest.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_11 = {
			files = {"MusicBattlePreparationsWastelands.ogg"},
			gain = 0.9,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_12 = {
			files = {"MusicBattlePreparationsWastelands.ogg"},
			gain = 0.9,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_13 = {
			files = {"MusicBattlePreparationsMountain.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_14 = {
			files = {"MusicBattlePreparationsForest.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_15 = {
			files = {"MusicBattlePreparationsWastelands.ogg"},
			gain = 0.9,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_16 = {
			files = {"MusicBattlePreparationsForest.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_17 = {
			files = {"MusicBattlePreparationsForest.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_18 = {
			files = {"MusicBattlePreparationsMountain.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_19 = {
			files = {"MusicBattlePreparationsMountain.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_20 = {
			files = {"MusicBattlePreparationsWastelands.ogg"},
			gain = 0.9,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_21 = {
			files = {"MusicBattlePreparationsWastelands.ogg"},
			gain = 0.9,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_22 = {
			files = {"MusicBattlePreparationsWastelands.ogg"},
			gain = 0.9,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_23 = {
			files = {"CB_PreBattle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_24 = {
			files = {"CB_PreBattle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_25 = {
			files = {"CB_PreBattle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_26 = {
			files = {"CB_PreBattle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_27 = {
			files = {"savage_music_desert_preparation.ogg"},
			gain = 0.4,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_28 = {
			files = {"Desert_Prep.ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_29 = {
			files = {"savage_music_desert_preparation.ogg"},
			gain = 0.4,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_30 = {
			files = {"Desert_Prep.ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_31 = {
			files = {"savage_music_desert_preparation.ogg"},
			gain = 0.4,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_32 = {
			files = {"Desert_Prep.ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_33 = {
			files = {"savage_music_jungle_preparation.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_34 = {
			files = {"Jungle_Prep.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_35 = {
			files = {"savage_music_jungle_preparation.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_36 = {
			files = {"Jungle_Prep.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_37 = {
			files = {"savage_music_jungle_preparation.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_38 = {
			files = {"savage_music_underground_preparation.ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_39 = {
			files = {"Underground_Prep.ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_40 = {
			files = {"savage_music_underground_preparation.ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_41 = {
			files = {"Underground_Prep.ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_42 = {
			files = {"Prebattle_Rising_Tides.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_43 = {
			files = {"Prebattle_Rising_Tides.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_44 = {
			files = {"Prebattle_Rising_Tides.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_45 = {
			files = {"Halloween_PreBattle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_46 = {
			files = {"Halloween_PreBattle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_47 = {
			files = {"Halloween_PreBattle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_48 = {
			files = {"Underground_Prep.ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_49 = {
			files = {"kro_bgmusic_t1_prebattle1_[master_25-08].ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_50 = {
			files = {"kro_bgmusic_t1_prebattle2[master_30-09].ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_51 = {
			files = {"kro_bgmusic_t1_prebattle1_[master_25-08].ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_52 = {
			files = {"kro_bgmusic_t1_prebattle2[master_30-09].ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_53 = {
			files = {"kro_bgmusic_t1_prebattle1_[master_25-08].ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_54 = {
			files = {"kro_bgmusic_t1_prebattle2[master_30-09].ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_55 = {
			files = {"kro_bgmusic_t2_prebattle1[master_30-09].ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_56 = {
			files = {"kro_bgmusic_t2_prebattle2[master_25-08].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_57 = {
			files = {"kro_bgmusic_t2_prebattle1[master_30-09].ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_58 = {
			files = {"kro_bgmusic_t2_prebattle2[master_25-08].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_59 = {
			files = {"kro_bgmusic_t2_prebattle1[master_30-09].ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_60 = {
			files = {"kro_bgmusic_t3_prebattle1[master_01-11].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_61 = {
			files = {"kro_bgmusic_t3_prebattle2[master_25-08].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_62 = {
			files = {"kro_bgmusic_t3_prebattle1[master_01-11].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_63 = {
			files = {"kro_bgmusic_t3_prebattle2[master_25-08].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_64 = {
			files = {"kro_bgmusic_t1_prebattle1_[master_25-08].ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_65 = {
			files = {"kro_bgmusic_t1_prebattle2[master_30-09].ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_66 = {
			files = {"kro_bgmusic_t1_prebattle1_[master_25-08].ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_67 = {
			files = {"kro_bgmusic_t1_prebattle1_[master_25-08].ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_68 = {
			files = {"kro_bgmusic_t1_prebattle1_[master_25-08].ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_69 = {
			files = {"kro_bgmusic_t2_prebattle1[master_30-09].ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_70 = {
			files = {"kro_bgmusic_t2_prebattle2[master_25-08].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_101 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t1_preparation1_v1.ogg"}
		},
		MusicBattlePrep_102 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t1_preparation2_v1.ogg"}
		},
		MusicBattlePrep_103 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t1_preparation1_v1.ogg"}
		},
		MusicBattlePrep_104 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t1_preparation2_v1.ogg"}
		},
		MusicBattlePrep_105 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t1_preparation1_v1.ogg"}
		},
		MusicBattlePrep_106 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t1_preparation2_v1.ogg"}
		},
		MusicBattlePrep_107 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t2_preparation1_vN.ogg"}
		},
		MusicBattlePrep_108 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t2_preparation2_v2.ogg"}
		},
		MusicBattlePrep_109 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t2_preparation1_vN.ogg"}
		},
		MusicBattlePrep_110 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t2_preparation2_v2.ogg"}
		},
		MusicBattlePrep_111 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t2_preparation1_vN.ogg"}
		},
		MusicBattlePrep_112 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t3_preparation1_v1.ogg"}
		},
		MusicBattlePrep_113 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t3_preparation2_v1.ogg"}
		},
		MusicBattlePrep_114 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t3_preparation1_v1.ogg"}
		},
		MusicBattlePrep_115 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t3_preparation1_v1.ogg"}
		},
		MusicBattlePrep_116 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t3_preparation3_v1.ogg"}
		},
		MusicBattlePrep_117 = {
			source_group = "MUSIC",
			gain = 0.5,
			loop = true,
			stream = true,
			files = {"kr5_update1_bgmusic_preparation1_v3.ogg"}
		},
		MusicBattlePrep_118 = {
			source_group = "MUSIC",
			gain = 0.6,
			loop = true,
			stream = true,
			files = {"kr5_update1_bgmusic_preparation1_v3.ogg"}
		},
		MusicBattlePrep_119 = {
			source_group = "MUSIC",
			gain = 0.6,
			loop = true,
			stream = true,
			files = {"kr5_update1_bgmusic_preparation1_v3.ogg"}
		},
		MusicBattlePrep_120 = {
			source_group = "MUSIC",
			gain = 0.6,
			loop = true,
			stream = true,
			files = {"kr5_update2_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_121 = {
			source_group = "MUSIC",
			gain = 0.5,
			loop = true,
			stream = true,
			files = {"kr5_update2_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_122 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_update2_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_123 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc1_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_124 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc1_bgmusic_preparation2_v2.ogg"}
		},
		MusicBattlePrep_125 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_dlc1_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_126 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_dlc1_bgmusic_preparation2_v2.ogg"}
		},
		MusicBattlePrep_127 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_dlc1_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_128 = {
			source_group = "MUSIC",
			gain = 0.6,
			loop = true,
			stream = true,
			files = {"kr5_update3_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_129 = {
			source_group = "MUSIC",
			gain = 0.5,
			loop = true,
			stream = true,
			files = {"kr5_update3_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_130 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_update3_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_131 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc2_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_132 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc2_bgmusic_preparation2_v1.ogg"}
		},
		MusicBattlePrep_133 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc2_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_134 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc2_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_135 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc2_bgmusic_preparation2_v1.ogg"}
		},
		MusicBattlePrep_136 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc3_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_137 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc3_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_138 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc3_bgmusic_preparation1_v1.ogg"}
		},
		MusicBattlePrep_139 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc3_bgmusic_preparation2_v1.ogg"}
		},
		MusicBattlePrep_140 = {
			source_group = "MUSIC",
			gain = 0.35,
			loop = true,
			stream = true,
			files = {"kr5_dlc3_bgmusic_preparation3_v1.ogg"}
		},
		MusicBattlePrep_1001 = {
			files = {"MusicBattlePreparationsMountain.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_1002 = {
			files = {"CB_PreBattle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_2000 = {
			files = {"savage_music_jungle_preparation.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_2001 = {
			files = {"Prebattle_Rising_Tides.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_2002 = {
			files = {"Jungle_Prep.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_2003 = {
			files = {"29431171705-1-192.ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_2004 = {
			files = {"Desert_Prep.ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattlePrep_3000 = {
			files = {"kro_bgmusic_t3_prebattle1[master_01-11].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_01 = {
			files = {"MusicBattleUnderAttackForest1.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_02 = {
			files = {"MusicBattleUnderAttackForest2.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_03 = {
			files = {"MusicBattleUnderAttackForest1.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_04 = {
			files = {"MusicBattleUnderAttackForest2.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_05 = {
			files = {"MusicBattleUnderAttackForest1.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_06 = {
			files = {"MusicBattleUnderAttackForest1.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_07 = {
			files = {"MusicBattleUnderAttackMountain.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_08 = {
			files = {"MusicBattleUnderAttackMountain.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_09 = {
			files = {"MusicBattleUnderAttackMountain.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_10 = {
			files = {"MusicBattleUnderAttackWastelands.ogg"},
			gain = 0.9,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_1000 = {
			files = {"MusicBattleUnderAttackForest1.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_11 = {
			files = {"MusicBattleUnderAttackWastelands.ogg"},
			gain = 0.9,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_12 = {
			files = {"KingdomRushWastelandVeznanCastle.ogg"},
			gain = 1,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_13 = {
			files = {"MusicBattleUnderAttackMountain.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_14 = {
			files = {"MusicBattleUnderAttackForest1.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_15 = {
			files = {"MusicBattleUnderAttackWastelands.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_16 = {
			files = {"MusicBattleUnderAttackForest1.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_17 = {
			files = {"MusicBattleUnderAttackForest2.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_18 = {
			files = {"MusicBattleUnderAttackMountain.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_19 = {
			files = {"MusicBattleUnderAttackMountain.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_20 = {
			files = {"MusicBattleUnderAttackWastelands.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_21 = {
			files = {"MusicBattleUnderAttackWastelands.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_22 = {
			files = {"MusicBattleUnderAttackWastelands.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_23 = {
			files = {"CB_Battle.ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_24 = {
			files = {"CB_Battle.ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_25 = {
			files = {"CB_Battle.ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_26 = {
			files = {"CB_Battle.ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_27 = {
			files = {"savage_music_desert_battle.ogg"},
			gain = 0.4,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_28 = {
			files = {"Desert_Battle.ogg"},
			gain = 0.4,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_29 = {
			files = {"savage_music_desert_battle.ogg"},
			gain = 0.4,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_30 = {
			files = {"Desert_Battle.ogg"},
			gain = 0.4,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_31 = {
			files = {"savage_music_desert_battle.ogg"},
			gain = 0.4,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_32 = {
			files = {"Desert_Battle.ogg"},
			gain = 0.4,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_33 = {
			files = {"Jungle_Battle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_34 = {
			files = {"savage_music_jungle_battle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_35 = {
			files = {"Jungle_Battle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_36 = {
			files = {"savage_music_jungle_battle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_37 = {
			files = {"Jungle_Battle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_38 = {
			files = {"savage_music_underground_battle.ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_39 = {
			files = {"Underground_Battle.ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_40 = {
			files = {"savage_music_underground_battle.ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_41 = {
			files = {"Underground_Battle.ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_42 = {
			files = {"Battle_Rising_Tides.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_43 = {
			files = {"Battle_Rising_Tides.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_44 = {
			files = {"Battle_Rising_Tides.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_45 = {
			files = {"Halloween_Battle.ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_46 = {
			files = {"Halloween_Battle.ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_47 = {
			files = {"Halloween_Battle.ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_48 = {
			files = {"Underground_Battle.ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_49 = {
			files = {"kro_bgmusic_t1_battle1_v4[master_03-11].ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_50 = {
			files = {"kro_bgmusic_t1_battle2[master_25-08].ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_51 = {
			files = {"kro_bgmusic_t1_battle1_v4[master_03-11].ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_52 = {
			files = {"kro_bgmusic_t1_battle2[master_25-08].ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_53 = {
			files = {"kro_bgmusic_t1_battle1_v4[master_03-11].ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_54 = {
			files = {"kro_bgmusic_t1_battle2[master_25-08].ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_55 = {
			files = {"kro_bgmusic_t2_battle1.ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_56 = {
			files = {"kro_bgmusic_t2_battle2[master_03-09].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_57 = {
			files = {"kro_bgmusic_t2_battle1.ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_58 = {
			files = {"kro_bgmusic_t2_battle2[master_03-09].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_59 = {
			files = {"kro_bgmusic_t2_battle1.ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_60 = {
			files = {"kro_bgmusic_t3_battle1[master_03-11].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_61 = {
			files = {"kro_bgmusic_t3_battle2[master_30-10].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_62 = {
			files = {"kro_bgmusic_t3_battle1[master_03-11].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_63 = {
			files = {"kro_bgmusic_t3_battle2[master_30-10].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_64 = {
			files = {"kro_bgmusic_t1_battle1_v4[master_03-11].ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_65 = {
			files = {"kro_bgmusic_t1_battle2[master_25-08].ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_66 = {
			files = {"kro_bgmusic_t1_battle1_v4[master_03-11].ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_67 = {
			files = {"kro_bgmusic_t1_battle1_v4[master_03-11].ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_68 = {
			files = {"kro_bgmusic_t1_battle1_v4[master_03-11].ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_69 = {
			files = {"kro_bgmusic_t2_battle1.ogg"},
			gain = 0.2,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_70 = {
			files = {"kro_bgmusic_t2_battle2[master_03-09].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_101 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t1_battle1_v1.ogg"}
		},
		MusicBattle_102 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t1_battle2_v1.ogg"}
		},
		MusicBattle_103 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t1_battle1_v1.ogg"}
		},
		MusicBattle_104 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t1_battle2_v1.ogg"}
		},
		MusicBattle_105 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t1_battle1_v1.ogg"}
		},
		MusicBattle_106 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t1_battle2_v1.ogg"}
		},
		MusicBattle_107 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t2_battle1_v1.ogg"}
		},
		MusicBattle_108 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t2_battle2_v2.ogg"}
		},
		MusicBattle_109 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t2_battle1_v1.ogg"}
		},
		MusicBattle_110 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t2_battle2_v2.ogg"}
		},
		MusicBattle_111 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t2_battle1_v1.ogg"}
		},
		MusicBattle_112 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t3_battle1_v1.ogg"}
		},
		MusicBattle_113 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t3_battle2_v1.ogg"}
		},
		MusicBattle_114 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t3_battle1_v1.ogg"}
		},
		MusicBattle_115 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t3_battle2_v1.ogg"}
		},
		MusicBattle_116 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t3_boss2_v1.ogg"}
		},
		MusicBattle_117 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_update1_bgmusic_battle1_v2.ogg"}
		},
		MusicBattle_118 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_update1_bgmusic_battle1_v2.ogg"}
		},
		MusicBattle_119 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_update1_bgmusic_battle1_v2.ogg"}
		},
		MusicBattle_120 = {
			source_group = "MUSIC",
			gain = 0.2,
			loop = true,
			stream = true,
			files = {"kr5_update2_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_121 = {
			source_group = "MUSIC",
			gain = 0.2,
			loop = true,
			stream = true,
			files = {"kr5_update2_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_122 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_update2_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_123 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_dlc1_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_124 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc1_bgmusic_battle2_v1.ogg"}
		},
		MusicBattle_125 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_dlc1_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_126 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc1_bgmusic_battle2_v1.ogg"}
		},
		MusicBattle_127 = {
			source_group = "MUSIC",
			gain = 0.25,
			loop = true,
			stream = true,
			files = {"kr5_dlc1_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_128 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_update3_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_129 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_update3_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_130 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_update3_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_131 = {
			source_group = "MUSIC",
			gain = 0.2,
			loop = true,
			stream = true,
			files = {"kr5_dlc2_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_132 = {
			source_group = "MUSIC",
			gain = 0.2,
			loop = true,
			stream = true,
			files = {"kr5_dlc2_bgmusic_battle2_v1.ogg"}
		},
		MusicBattle_133 = {
			source_group = "MUSIC",
			gain = 0.2,
			loop = true,
			stream = true,
			files = {"kr5_dlc2_bgmusic_battle3_v1.ogg"}
		},
		MusicBattle_134 = {
			source_group = "MUSIC",
			gain = 0.2,
			loop = true,
			stream = true,
			files = {"kr5_dlc2_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_135 = {
			source_group = "MUSIC",
			gain = 0.2,
			loop = true,
			stream = true,
			files = {"kr5_dlc2_bgmusic_battle2_v1.ogg"}
		},
		MusicBattle_136 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc3_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_137 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc3_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_138 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc3_bgmusic_battle1_v1.ogg"}
		},
		MusicBattle_139 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc3_bgmusic_battle2_v1.ogg"}
		},
		MusicBattle_140 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc3_bgmusic_battle3_v1.ogg"}
		},
		MusicBossFight_140 = {
			source_group = "MUSIC",
			gain = 0.3,
			loop = true,
			stream = true,
			files = {"kr5_dlc3_bgmusic_bossBattle_v1.ogg"}
		},
		MusicBattle_1001 = {
			files = {"MusicBattleUnderAttackMountain.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_1002 = {
			files = {"CB_Battle.ogg"},
			gain = 0.5,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_2000 = {
			files = {"Jungle_Battle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_2001 = {
			files = {"Battle_Rising_Tides.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_2002 = {
			files = {"Jungle_Battle.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_2003 = {
			files = {"33145294233-1-192.ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_2004 = {
			files = {"savage_music_desert_battle.ogg"},
			gain = 0.4,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBattle_3000 = {
			files = {"kro_bgmusic_t3_battle1[master_03-11].ogg"},
			gain = 0.3,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBossFight = {
			files = {"MusicBossFight.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBossPreFight = {
			files = {"boss_prefight.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicBossPreFight63 = {
			files = {"kro_bgmusic_bossfight_prebattle_v2[master_03-11].ogg"},
			gain = 0.7,
			source_group = "MUSIC",
			stream = true
		},
		MusicBossPreFightEnd = {
			files = {"Finalboss_prefight.ogg"},
			gain = 0.7,
			source_group = "MUSIC",
			stream = true
		},
		MusicBossFight_106 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t1_boss_v2.ogg"}
		},
		MusicBossFight_111 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t2_boss_v1.ogg"}
		},
		MusicBossFight_115 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t3_boss1_v1.ogg"}
		},
		MusicBossFight_116 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_t3_boss2_v1.ogg"}
		},
		MusicBossFight_119 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_update1_bgmusic_bossBattle_v2.ogg"}
		},
		MusicBossFight_122 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_update2_bgmusic_bossBattle_v1.ogg"}
		},
		MusicBossFight_127 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_dlc1_bgmusic_bossBattle_v1.ogg"}
		},
		MusicBossFight_130 = {
			source_group = "MUSIC",
			gain = 0.4,
			loop = true,
			stream = true,
			files = {"kr5_update3_bgmusic_bossBattle_v1.ogg"}
		},
		MusicBossFight_135 = {
			source_group = "MUSIC",
			gain = 0.2,
			loop = true,
			stream = true,
			files = {"kr5_dlc2_bgmusic_bossBattle_v1.ogg"}
		},
		MusicCredits = {
			files = {"kro_bgmusic_creditos[master].ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicEndCredits = {
			files = {"MusicEndCredits.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicEndVictory = {
			files = {"Victory_Theme.ogg"},
			gain = 0.7,
			source_group = "MUSIC",
			stream = true
		},
		MusicHalloweenMoon = {
			files = {"Halloween_Moon.ogg"},
			gain = 0.7,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicMainMenu = {
			files = {"MusicMainMenu.ogg"},
			gain = 1,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicMap1 = {
			files = {"MusicMap.ogg"},
			gain = 0.8,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicMap2 = {
			files = {"Map_Theme_1.ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicMap3 = {
			files = {"kre_bgmusic_map[master_25-08].ogg"},
			gain = 0.6,
			loop = true,
			source_group = "MUSIC",
			stream = true
		},
		MusicMap5 = {
			source_group = "MUSIC",
			gain = 0.6,
			loop = true,
			stream = true,
			files = {"kr5_bgmusic_atlas_v1.ogg"}
		},
		MusicSuspense = {
			files = {"MusicSuspense.ogg"},
			gain = 0.7,
			source_group = "MUSIC",
			stream = true
		},
		PirateTowerTaunt1 = {
			files = {"Pirate_Tower_01a.ogg"},
			gain = 1,
			loop = false,
			source_group = "TAUNTS"
		},
		PirateTowerTaunt2 = {
			files = {"Pirate_Tower_02b.ogg"},
			gain = 1,
			loop = false,
			source_group = "TAUNTS"
		},
		ReinforcementTaunt = {
			files = {"Reinforcements_Event1.ogg", "Reinforcements_Event2.ogg", "Reinforcements_Event3.ogg", "Reinforcements_Event4.ogg"},
			gain = 0.6,
			ignore = 1,
			loop = false,
			mode = "sequence",
			source_group = "TAUNTS"
		},
		SasquashRally = {
			files = {"Sound_TowerSoldierSasquashRally.ogg"},
			gain = 0.8,
			loop = false,
			source_group = "TAUNTS"
		},
		SasquashReady = {
			files = {"Sound_TowerSoldierSasquashReady.ogg"},
			gain = 0.8,
			loop = false,
			source_group = "TAUNTS"
		},
		SaurianSavantPortalLoop = {
			files = {"savant_portal_loop.ogg"},
			gain = 0.3,
			loop = true,
			ref_counted = true,
			source_group = "REFCOUNTED"
		},
		ShotgunSound = {
			files = {"Sound_Shootgun.ogg"},
			gain = {0.65, 0.8},
			loop = false,
			source_group = "BULLETS"
		},
		ShrapnelSound = {
			files = {"Sound_Shrapnel.ogg"},
			gain = 0.12,
			loop = false,
			source_group = "BULLETS"
		},
		SoldierDruidBearRallyChange = {
			files = {"Viking_Transform.ogg"},
			gain = 1,
			loop = false,
			source_group = "TAUNTS"
		},
		TenShiAttack1 = {
			files = {"kr1_sfx_tenshi_basicAttack1_var1_v1.ogg", "kr1_sfx_tenshi_basicAttack1_var2_v1.ogg", "kr1_sfx_tenshi_basicAttack1_var3_v1.ogg"},
			gain = 0.6,
			loop = false,
			mode = "random",
			source_group = "BULLETS"
		},
		TenShiAttack2 = {
			files = {"kr1_sfx_tenshi_attack2_v1.ogg"},
			gain = 0.6,
			loop = false,
			source_group = "BULLETS"
		},
		TenShiBuffedBombAttack = {
			files = {"kr1_sfx_tenshi_buffedBombAttack_3hits_v1_op2.ogg"},
			gain = 1,
			loop = false,
			source_group = "SFX"
		},
		TenShiBuffedBombAttackLong = {
			files = {"kr1_sfx_tenshi_buffedBombAttack_6hits_v1_op2.ogg"},
			gain = 1,
			loop = false,
			source_group = "SFX"
		},
		TenShiBuffedSpinAttack = {
			files = {"kr1_sfx_tenshi_buffedSpinAttack_2hits_v1.ogg", "kr1_sfx_tenshi_buffedSpinAttack_3hits_v1.ogg"},
			gain = 1,
			loop = false,
			mode = "random",
			source_group = "SFX"
		},
		TenShiDeathSfx = {
			files = {"kr1_sfx_tenshi_death_v1.ogg"},
			gain = 1,
			loop = false,
			source_group = "SFX"
		},
		TenShiRainOfFireEnd = {
			files = {"kr1_sfx_tenshi_rainOfFire_loop-End_v1_10db.ogg"},
			gain = 1,
			loop = false,
			source_group = "SFX"
		},
		TenShiRainOfFireStart = {
			files = {"kr1_sfx_tenshi_rainOfFire_start_mix.ogg"},
			gain = 1,
			loop = false,
			source_group = "SFX"
		},
		TenShiRespawn = {
			files = {"TenShi_respawn_01d.ogg"},
			gain = 0.7,
			loop = false,
			source_group = "TAUNTS"
		},
		TenShiTaunt = {
			files = {"TenShi_02c.ogg", "TenShi_01c_and_a.ogg", "TenShi_01a_mmh.ogg", "TenShi_01a_mmh.ogg", "TenShi_01a_mmh.ogg"},
			gain = 0.7,
			ignore = 1,
			loop = false,
			mode = "random",
			source_group = "TAUNTS"
		},
		TenShiTauntBuffed = {
			files = {"TenShi_buffed_01c.ogg", "TenShi_buffed_02b.ogg", "TenShi_buffed_01a_laugh_only.ogg"},
			gain = 0.7,
			ignore = 1,
			loop = false,
			mode = "random",
			source_group = "TAUNTS"
		},
		TenShiTauntIntro = {
			files = {"Level_up.ogg"},
			gain = 0.7,
			loop = false,
			source_group = "TAUNTS"
		},
		TenShiTauntSelect = {
			files = {"TenShi_02c.ogg"},
			gain = 0.8,
			loop = false,
			source_group = "TAUNTS"
		},
		TenShiTeleportSfx = {
			files = {"kr1_sfx_tenshi_teleport_v1_15db.ogg"},
			gain = 1,
			loop = false,
			source_group = "SFX"
		},
		TenShiTransformToBuffed = {
			files = {"TenShi_transformation_01d.ogg", "TenShi_transformation_02c.ogg"},
			gain = 1,
			loop = false,
			mode = "sequence",
			source_group = "TAUNTS"
		},
		TenShiTransformToBuffedSfx = {
			files = {"kr1_sfx_tenshi_transformToBuffed_op1_v1.ogg"},
			gain = 1,
			loop = false,
			source_group = "SFX"
		},
		TenShiTransformToNormal = {
			files = {"TenShi_01a_mmh.ogg"},
			gain = 1,
			loop = false,
			mode = "sequence",
			source_group = "TAUNTS"
		},
		TenShiTransformToNormalSfx = {
			files = {"kr1_sfx_tenshi_transformToNormal_v1.ogg"},
			gain = 1,
			loop = false,
			source_group = "SFX"
		},
		juggernaut_punch = {
			files = {"kr4_sfx_juggernaut_punch_var1_v1.ogg"},
			gain = 1,
			loop = false,
			source_group = "SFX"
		},
		kr4_tower_wickedsisters_attack_v1 = {
			files = {"kr4_tower_wickedsisters_attack_v1.ogg"},
			gain = 1,
			loop = false,
			source_group = "SFX"
		},
		swamp_thing_bomb_explosion = {
			files = {"Hero_Karkinos_Cannon_explosion.ogg"},
			gain = 1,
			loop = false,
			source_group = "SPECIALS"
		},
		swamp_thing_bomb_shot = {
			files = {"swamp-thing-shot.ogg"},
			gain = 1,
			loop = false,
			source_group = "SPECIALS"
		}
	}
}
